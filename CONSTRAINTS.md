...
| U21 | Cross-action interference in shared state is load-bearing for exploration | When a single learning model absorbs transitions from ALL actions, cross-action interference prevents convergence. Persistent prediction error drives diverse actions drives exploration. Separating the model per action eliminates interference, prediction becomes 12x easier, the model converges, and exploration dies. The "contamination" from conflated action-conditional transitions IS the structured noise (U12) that sustains exploration. This is a specific instance of U13 (additions hurt): per-action specialization is an "improvement" that destroys a load-bearing side effect. Same pattern as S2 (F.normalize creating Goldilocks noise zone). | Phase 2: TemporalPrediction shared W unique=1536 vs PerAction W unique=991. pred_err 0.86 vs 0.07 at 1K steps. |
