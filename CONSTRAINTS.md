...
**Level 2 reward disconnect — confirmed from 4 independent angles (Steps 486-639).** Level 2's reward is in a region the agent hasn't reached at 740K steps with 439 cells (k=12) or at 500K with 1149 cells (k=16). Four independent approaches all fail: (1) **Coverage** (486-492): edge manipulation, death avoidance/seeking, stochastic exploitation, finer partitioning — reachable set grows sublinearly but L2 remains disconnected. (2) **Eigenform** (620-629): self-observation is reflective only — computes functions of visited states, can't reach unvisited states. (3) **Delta/stale** (630-637, 8 experiments): graph-derived signals at any granularity — k=16 too fine, k=4 too coarse, binary over-collapses, per-edge stale 12-54% inert. (4) **Environmental events** (638-639): sparse environmental signals (death ~1.3%, large_diff ~5.2%) are seed-dependent — benefit depends on game topology per seed. ALL converge: L2 requires forward prediction, not retrospective penalty. The graph stores the past; L2 requires the future (Proposition 13).
...
| U28 | Signal sparsity determines utility for argmin modification | **PROVISIONAL** | LSH only (Steps 581d, 630-639, 18 experiments). Death penalty (~5% of edges) → 4/5, helps. Delta PREFER (66-98%) → hurts. Frontier gradient (94-98%) → hurts. Per-edge stale (12-54%) → inert. Environmental events (1-5%) → seed-dependent. Threshold: ~10%. Below: sparse signal can guide argmin; above: indistinguishable from noise because too many argmin decisions are perturbed. Related to noisy TV problem (Burda et al. 2018) but different mechanism: not prediction-error curiosity traps but argmin perturbation density. Single-family evidence (LSH). |
...
3. **Purposeful exploration (I6, I9):** All navigation is stochastic coverage with argmin bias. 6 targeted strategies (Steps 477-482) all worse than argmin — failures were mechanism-specific (noise, locality traps, noisy TV), not categorical. 10 further experiments (Steps 630-639) tested graph-derived and environmental signals as argmin modifications: all fail or are seed-dependent. **Sparsity finding (U28, Steps 581d + 630-639):** only signals firing on <~10% of edges help navigation; dense signals (>10%) are indistinguishable from noise on argmin because they perturb too many decisions. Death penalty (~5%) works: sparse, negative, per-edge. Delta (66-98%), frontier (94-98%), stale (12-54%) all fail. Environmental events (1-5%) are seed-dependent. The useful signal space for argmin modification is too narrow to bridge the L2 gap — L2 requires a fundamentally different exploration mechanism, not argmin + auxiliary signal. Previous attractor hypothesis (2026-03-21, downgraded by Hart) subsumed: "attractors vs non-attractors" was incomplete; signal density is the explanatory axis. Related to noisy TV problem (Burda et al. 2018) but distinct mechanism.
