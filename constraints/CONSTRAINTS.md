...
| PB30 | Simplicity observation: adding complexity to winning architecture degrades both sides. Defense: v22 (multi-scale) killed v21's signal, v23 (anti-oscillation) killed v21's signal. Prosecution: v21 (alpha-projected) killed v20's signal. Observed in 2 families (n=2 defense, n=1 prosecution). Post-hoc explanation: no optimizer to fit extra parameters in R1-R6 setting. Untested. | **PROVISIONAL** (observed in 2 families, n=3 total. Post-hoc explanation untested.) |

### Summary

Prediction transfer region non-empty (PB5). Navigation transfer region empty. Gap is structural (Prop 21). **0% wall = TWO failure modes (PB27):** Mode 1 = noise (dead, Step 1081). Mode 2 = oscillation — actions have directional effects but game state bounces (Step 1082, 30+ experiments). Each debate side solved 1 ARC game at L1 (different games, different draws): defense v21 reactive ARC=0.30, prosecution v20 attention-trajectory ARC=0.004 (PB26 approximate parity at L1). Simplicity observed but not confirmed: complexity degrades both sides (PB30, n=3). Cascade superseded, untested under ARC scoring (PB28). ALL BANS LIFTED. **Debate scoring = ARC Prize method. Distinct architectures mandated (Jun, 2026-03-25).**
