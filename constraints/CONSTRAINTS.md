...
Prediction transfer region non-empty (PB5). Navigation transfer region empty. Gap is structural (Prop 21). Debate v3 sprint (Steps 1042-1082, 38+ experiments): ℓ_π ≈ ℓ₁ at L1 when dimensionality controlled (PB26). MI detection = best L1 method (PB29). **0% wall = TWO failure modes (PB27):** Mode 1 = noise (dead). Mode 2 = OSCILLATION — actions have directional effects but don't accumulate. Needs state-dependent action selection (Step 1082). Sequence evolution is the wrong architecture. L2+ = 0 reliably. ALL BANS LIFTED (Jun, 2026-03-25). **Debate scoring = ARC Prize method: action efficiency vs human, squared, averaged (Jun, 2026-03-25).**
