...
| PB26 | ℓ_π (encoding modification) vs ℓ₁ (parameter modification). Early sprint: 2-2-1, confounded. Step 1067: v14 (ℓ_π) = 80%/70%/0% vs v16 (ℓ₁ 4096D) = 0%/0%/0% — dimensionality confound (v16 SPSA on 4096D intractable). Step 1069: v17 (ℓ₁ 64D) NO ZEROS — draw luck (Step 1071: v17 gets 2 zeros on different draw). **Steps 1070-1071: controlled runs show both sides hit ~1/3 0% wall. Approximate parity when dimensionality controlled.** The bottleneck is opaque games, not ℓ-level. ANIL direction (Raghu et al., ICLR 2020) not confirmed in this setting. | **PROVISIONAL** (Steps 1042-1071. Approximate parity. 0% wall structural, affects both equally.) |
...
| PB28 | ~~Adaptive cascade = best architecture.~~ **SUPERSEDED by Steps 1084-1087.** Post-cascade architectures outperform: defense v21 (reactive switching, ARC=0.2973) and prosecution v20 (attention-trajectory, ARC=0.0037) both solve 1/3 ARC games. Cascade solved 0 ARC games autonomously (L1 only, no ARC score). Jun mandated distinct architectures (2026-03-25). | **REVISED** (Steps 1084-1087 supersede 1042-1065. Cascade is dead.) |
| PB30 | **Simplicity is load-bearing (Steps 1084-1087).** Adding complexity to winning architecture DEGRADES both sides: defense v22 (multi-scale) KILLED v21's signal, prosecution v21 (alpha-projected) KILLED v20's signal. Confirmed across 2 architecturally distinct families (reactive switching vs attention-trajectory). In R1-R6 constrained setting (no optimizer), extra parameters become noise — no training signal to fit them. Simplicity bias (Sorscher et al. 2019 direction) but novel in unsupervised/no-optimizer setting. | **PROVISIONAL** (2 families confirm, but only 1 experiment each direction. Needs more data.) |

### Summary

Prediction transfer region non-empty (PB5). Navigation transfer region empty. Gap is structural (Prop 21). Debate v3 (Steps 1042-1087, 43+ experiments): Both sides solve 1/3 ARC games at L1 — defense v21 (reactive, ARC=0.30) leads 80x on efficiency vs prosecution v20 (attention-trajectory, ARC=0.004). **Simplicity is load-bearing (PB30):** adding complexity degrades both sides (defense v22 killed, prosecution v21 killed). **0% wall ≈ 2/3 games (PB27).** Cascade is dead (PB28 revised). ALL BANS LIFTED. **Debate scoring = ARC Prize method (Jun, 2026-03-25). Distinct architectures mandated (Jun, 2026-03-25).**
