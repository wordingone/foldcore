...
FINDING (Steps 778-784 v2): **LS20 is action-0 dominated.** High L1: action-0-persistent mechanisms (random seed 0=164, momentum=217, cycling cold=226). Zero L1: action-diversifying mechanisms (prediction-contrast, ensemble, Hebbian, round-robin). LS20 is the WRONG test environment for post-ban action selection. Pivoting to FT09 (68 actions, diverse clicks needed). Game-specificity confound identified.
PIVOT (2026-03-23): Critical path experiments move to FT09. LS20 action-0 dominance makes it uninformative for testing diverse exploration mechanisms. FT09 requires discovering which of 68 actions (clicks) work → diverse exploration SHOULD help.
