...
CURRENT STEP: **Steps 1112-1114: 0% wall is STRUCTURAL, not mechanism.** All 3 experiments ARC=0 (adaptive click, random explore, differential encoding). 9/11 experiments since v30 = ARC=0. Both sides hit same wall: distance-to-initial progress metric fails on ~60% of random draws. **The goal function (distance-to-initial) is the shared bottleneck.** Games where progress = moving AWAY from initial are unsolvable by either ℓ₁ or ℓ_π with current objective. Debate must shift from encoding to goal function. 1114+ experiments.
