...
| **510** | **CIFAR centroid count sweep (k=50-1000, NMI)** | — | **NMI=0.34** | **Encoding HAS class signal.** NMI monotonically climbs: k=50→0.145, k=100→0.188, k=200→0.230, k=500→0.311, k=1000→0.344. At k=1000: 439/1000 centroids >50% class-pure, purity=0.587. Trend still climbing. **The wall is the aggregation mechanism, not the encoding.** Centroids contain class info but argmin doesn't aggregate by class. Hierarchical clustering of centroids (meta-clusters) could extract class signal without labels. |
