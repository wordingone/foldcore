...
| **499** | **FT09 action space diagnostic** | — | **17%** | **FT09 CRACKED: ACTION6 is complex (x,y click). 5/6 actions equivalent per cell (non-complex → deterministic transitions). ACTION6 with argmax click → self-loop.** 192 unique (cell,action,next_cell) triples, fully deterministic. ZERO reward in 30K steps. 32-cell graph = "no-click" subgraph only. Codebook's 69-class encoding worked by exploring 69 spatial click regions via ACTION6. FT09 is NOT topology-gated — it's I3-gated (action space discovery). The substrate must discover that ACTION6 with DIVERSE click positions produces different outcomes. |
