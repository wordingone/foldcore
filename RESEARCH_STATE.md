...
| **478** | **Destination-novelty action selection on LSH** | — | **25%** | **1/10. Much worse.** Agent trapped in local neighborhoods — destination scoring creates loops. sig_q=0.71 (high) but occ=4.7% (collapsed). 22 experiments (466-478): graph+argmin mechanism fully mapped. The 6/10 ceiling is fundamental. NOT in encoding, partitioning, action selection, multi-hash, or destination tracking. |
