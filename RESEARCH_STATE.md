...
| **506** | **CHAIN: CIFAR-100 → LS20 (frozen centroids)** | — | **1%/0** | **CHAIN FAIL. CIFAR centroids break LS20 navigation (0/1 at 50K vs 5/10 baseline).** Phase 1 CIFAR: 1.10% acc (chance), 300 centroids, 99 unique cells. Phase 2 LS20: 0/1, go=387, cells=99/300 — ARC frames map to same 99 CIFAR cells. Phase 3 CIFAR: 0.90% acc (unchanged). **Catastrophic forgetting does NOT occur** — CIFAR acc stable. **Interference IS the problem** — shared centroids impose wrong partition across domains. U3 (growth-only) satisfied but accumulated structure is HOSTILE to new domain. |
