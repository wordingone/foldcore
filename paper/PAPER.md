...
| 868b | Softmax\_01 varied seeds | 800b + softmax $T$=0.1, substrate\_seed=seed | N/A | Running | Validating 379/seed |
...
**Step 895c (correct spec): R3\_cf partial PASS on LS20.** Softmax $T$=0.1 action selection + $\alpha$-weighted delta (L2 norm) decouples navigation from R3. Results on LS20: warm (W+$\alpha$ transfer, fresh $\delta$) = 77.8/seed at 10K steps, cold = 23.0/seed ($\Delta$=+54.8). FT09: L1=0 at 5K steps ($\alpha$ confirms dims [60,51,52], conc=9.56×). Three findings: (1) R3\_cf warm $>$ cold on LS20 — warm $\alpha$ (pre-concentrated at 16.5×) navigates better than cold $\alpha$ (which either hyper-concentrates to 61.9× or stays diffuse); (2) cold navigation below random (23.0 $<$ 36.4) due to premature $\alpha$ concentration distorting delta; (3) FT09 L1=0 because 5K steps/68 actions is insufficient for delta differentiation (each action gets $\sim$73 visits before softmax concentrates). The LS20 warm result is the first positive L1 signal with $\alpha$ self-modification; it is 2× random but 5× below 800b baseline, indicating $\alpha$ weighting is helping but not yet dominant over the delta dynamics.
