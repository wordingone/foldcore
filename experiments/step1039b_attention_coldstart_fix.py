"""
Step 1039b -- Attention-Gated l_pi R3 (cold-start fix)

Fix for 1039's cold-start bug: uniform attention -> gated_freq proportional
to raw_freq -> same argmax -> advantage=0 -> attention never moves.

FIX: At end of warmup, initialize attention from change_map (interactive pixels
get high attention, non-interactive get low). Reset gated_freq to start fresh
with meaningful non-uniform attention. This breaks proportionality and enables
the advantage signal to differentiate.

This fix is game-agnostic: change_map is computed from interaction, not game
knowledge. Any game has pixels that change and pixels that don't.
...
Architecture (ONE system, ONE config, NO bootloader):
  ...
  attention[64,64]      - encoding gate in [0.01, 1.0], initialized from change_map
...
def main():
    ...
    print("=== Step 1039b: Prosecution -- Attention-Gated l_pi R3 (cold-start fix) ===")
