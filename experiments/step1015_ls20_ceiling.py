"""
Step 1015 — LS20 Per-Game Ceiling Re-verification (Direction 2, Sub-mode A).

SUBSTRATE: Full prescribed pipeline from Steps 572j-572u:
  - 674+running-mean (frozen bootloader)
  - Level-aware background modeling
  - Object detection via isolated CC
  - Dead reckoning + state estimation
  - LSH graph + per-state argmin (all bans lifted — Direction 2)

GAME: LS20/9607627b (current version)
BUDGET: 360s/seed × 5 seeds = 30 min total
TARGET: L1-L3 confirmed (minimum). L4+ tracked if reached.
KEY QUESTION: How many total levels does LS20 have?

Direction 2: ALL bans suspended (Jun directive 2026-03-24).
  - Codebook ban: LIFTED
  - Graph ban: LIFTED
...
# ─── Substrate (SubDual from 572u — unmodified) ───
...
def fire_level_handlers(sub, cl, prev_cl, step, seed, go,
                        l1_step_ref, l2_first_ref, l3_steps_ref, lN_events_ref):
    ...
    elif cl > 3 and prev_cl < cl:
        # L4+ — no substrate handler; substrate uses L3 state (best available)
        lN_events_ref.append((cl, step))
        print(f"  s{seed} L{cl}@{step}! [BEYOND L3] go={go}", flush=True)
...
def main():
    ...
    print("=" * 70)
    print("STEP 1015 — LS20 Per-Game Ceiling (Direction 2, Sub-mode A)")
    print("Substrate: 572u SubDual (LSH+BFS+dead-reckoning, all bans lifted)")
    print("Budget: 360s/seed × 5 seeds = 30 min")
    print("=" * 70)

    n_seeds = 5; per_seed_cap = 360; R = []; t_start = time.time()

    for seed in range(n_seeds):
        print(f"\nseed {seed}:", flush=True)
        env = mk(); sub = SubDual(seed=seed * 1000)
        obs = env.reset(seed=seed)
        l1_step = [None]; l2_first = [None]; l3_steps = []; lN_events = []
        ...
        for step in range(1, 2_000_001):
            ...
            if prev_cl == -1:
                fire_level_handlers(sub, cl, -1, step, seed, go,
                                    l1_step, l2_first, l3_steps, lN_events)
                prev_cl = cl
            elif cl > prev_cl:
                fire_level_handlers(sub, cl, prev_cl, step, seed, go,
                                    l1_step, l2_first, l3_steps, lN_events)
                prev_cl = cl
            else:
                prev_cl = cl
            ...
            if time.time() - t_seed > per_seed_cap:
                print(f"  cap @{step} go={go} l2c={sub.l2_count} l3c={sub.l3_count} "
                      f"max_cl={max_cl_seen} wall={sub._puq_wall_frozen} "
        ...
        R.append({'seed': seed, 'l2c': sub.l2_count, 'l3c': sub.l3_count,
                  'l3_steps': l3_steps, 'lN_events': lN_events,
                  'max_cl': max_cl_seen,
                  'puq_wall': sub._puq_wall_frozen,
                  'bfs_fails': sub.puq_bfs_fails, 'lhs_ap': sub._puq_lhs_ap,
                  'deaths': sub._puq_death_count, 'wp_idx': sub._puq_wp_idx})

    print(f"\n{'=' * 70}")
    print("STEP 1015 RESULTS:")
    l3_wins = sum(1 for r in R if r['l3c'] > 0)
    max_level_seen = max(r['max_cl'] for r in R) if R else 0
    for r in R:
        beyond = f" | L4+:{r['lN_events'][:3]}" if r['lN_events'] else ""
        print(f"  s{r['seed']}: L2={r['l2c']} L3={r['l3c']} max_cl={r['max_cl']} "
              f"wall={r['puq_wall']} deaths={r['deaths']} "
              f"bfs_f={r['bfs_fails']} lhs={r['lhs_ap']} "
              f"wp={r['wp_idx']} L3@{r['l3_steps'][:3] if r['l3_steps'] else []}{beyond}", flush=True)
    print(f"L3={l3_wins}/5  max_level_ever={max_level_seen}  "
          f"total_t={time.time()-t_start:.1f}s", flush=True)
    print("STEP 1015 DONE")
