"""
Step 544 -- Recode WITHOUT centering on LS20.

Step 542: Recode (centered) L1=5/5. Step 543: chain KILLED — centering removes
L2-scale separation between CIFAR and LS20 domains, causing node sharing.

Test: does removing centering from enc() preserve LS20 navigation?
If yes -> centering is not required at k=16 -> uncentered chain (Step 545) viable.
If 0/5 -> centering IS necessary for LSH navigation, chain needs different approach.

enc() change: remove x - x.mean(). Values now in [0, 1] (not zero-mean).
LS20 frames: mean ~0.03 (mostly black with colored sprites). CIFAR: mean ~0.5.

Everything else unchanged from Step 542: k=16, argmin, passive refinement.
5 seeds, 500K steps, 5-min cap per seed.

Predictions: 3/5 L1. Kill: 0/5.
"""
import numpy as np
import time
import sys
...
def enc(frame):
    """Avgpool16, NO centering. Values in [0, 1]."""
...
def t0():
    ...
    x1_ = (sub2.C[(0, 0, 2)][0] / 50).astype(np.float32)
    assert int(sub2.ref[0] @ x0 > 0) != int(sub2.ref[0] @ x1_ > 0), \
        "hyperplane should separate children"

    # Verify uncentered enc() produces values in [0, 1]
    frame_mock = [np.random.randint(0, 16, (64, 64))]
    xenc = enc(frame_mock)
    assert xenc.shape == (256,), f"enc shape: {xenc.shape}"
    assert xenc.min() >= 0.0 and xenc.max() <= 1.0, \
        f"uncentered enc out of [0,1]: min={xenc.min():.3f} max={xenc.max():.3f}"
...
def main():
    ...
    if l1n == 0:
        print("KILL: 0/5 L1. Centering required for LSH navigation.")
    elif l1n < 5:
        print(f"PARTIAL: {l1n}/5 L1. Centering helps but not required.")
    else:
        print(f"CLEAN: 5/5 L1. Centering not required at k=16.")
