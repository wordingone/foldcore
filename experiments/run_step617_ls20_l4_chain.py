"""
Step 617 — 572u base + L3/L4 bootstrap chain. NO G resets.

Root cause of 606-616 regression: G reset in on_l1/on_l2/on_l3 + wrong constants.
Fix: start from 572u (known-good: L2=5/5, L3=5/5), add L4 bootstrap only.

Changes from 572u:
  - game_level=3 support (L4 tracking)
  - L3 bootstrap: _update_bg builds puq wall_set mid-run at game_level==2
    (once l2_count>=N_MAP_PUQ AND l3_frames>=WARMUP) [from 612]
  - L4 bootstrap: _update_bg builds l4_targets at game_level==3
    (once l3_count>=N_MAP_L4 AND l4_frames>=WARMUP) [from 613]
  - on_l3(): now sets game_level=3
  - on_l4(): new handler
  - act(): L4 cluster targeting block added; L0 cluster elif gated on game_level==0
  - NO G resets anywhere (confirmed correct from 572u)

2 seeds x 300s. Signal: L3 >= 1/2 (confirms 572u base preserved). L4 >= 1/2 = win.
"""
import numpy as np
import time
import sys
...
def main():
    ...
    t_start = time.time()
    print("Step 617: LS20 L4 chain (572u base + L3/L4 bootstrap)", flush=True)
    print(f"  Base: 572u (L2=5/5, L3=5/5). NO G resets.", flush=True)
    print(f"  Added: L3 bootstrap (mid-run puq wall_set), L4 bootstrap (cluster targets)", flush=True)
    print(f"  Signal: L3>=1/2 (base preserved). L4>=1/2 = win.", flush=True)
    ...
    print(f"\n{'='*60}", flush=True)
    print(f"Step 617: LS20 L4 chain", flush=True)
    ...
    if l4_wins >= 1:
        ...
        print("  L3 not reached. 572u base broken — check game_level logic.", flush=True)
