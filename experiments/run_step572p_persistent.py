"""
Step 572p — Fix for persistent level in LS20 scorecard session.

Root cause (confirmed by 572o probe):
  After env.reset(seed=seed) within the SAME scorecard, the game starts at the
  HIGHEST LEVEL PREVIOUSLY REACHED, not at cl=0. Once puq (cl=2) is reached,
  every subsequent reset starts at cl=2 with 129-step energy budget.

  572m's `continue` fix was based on wrong assumption (game resets to cl=0).
  572k worked because spurious on_l2() fired for each puq start (prev_cl=1
  from mgu terminal → on_l2() fired even for terminal frame in puq).

Fix: use prev_cl=-1 sentinel after done. On first step of new game, call
on_lX() directly based on starting cl (no chain assumption needed).

Puq navigation unchanged from 572l/572m:
  Phase: gic(x1) -> kdy(x3, kdj-diff detection) -> lhs -> L3
  N_MAP_PUQ=30 puq episodes before building puq wall set from l3_mode
"""
import numpy as np
import time
import sys
...
class SubDual:
    ...
    def _puq_dr_action(self):
        ...
        if self._puq_phase == 'gic': goal = PUQ_GIC_GRID
        elif self._puq_phase == 'kdy': goal = PUQ_KDY_GRID
        else: goal = PUQ_LHS_GRID
        ...
        if len(self._puq_bfs_path) == 1: return None
...
def fire_level_handlers(sub, cl, prev_cl, step, seed, go, l1_step_ref, l2_first_ref, l3_steps_ref):
    """Fire appropriate level handlers. Handles both transitions and game-start-at-level."""
...
def main():
    ...
    for seed in range(n_seeds):
        ...
        prev_cl = -1  # -1 = sentinel: fire handler on first non-done step

        for step in range(1, 200_001):
            if obs is None:
                obs = env.reset(seed=seed); sub.on_reset(); prev_cl = -1; continue
            ...
            if done:
                go += 1; obs = env.reset(seed=seed); sub.on_reset(); prev_cl = -1
                continue

            cl = info.get('level', 0) if isinstance(info, dict) else 0

            if prev_cl == -1:
                # First step of new game — fire handler for starting level
                fire_level_handlers(sub, cl, -1, step, seed, go,
                                    l1_step, l2_first, l3_steps)
                prev_cl = cl
            elif cl > prev_cl:
        ...
        R.append(dict(seed=seed, l1=l1_step[0], l2=l2_first[0], l3_count=sub.l3_count,
    ...
    for r in R:
        tag = (f"L3x{r['l3_count']}" if r['l3_count'] > 0 else
               f"L2x{r['l2_count']}" if r['l2'] else
               "L1" if r['l1'] else "---")
    ...
    if l3n > 0:
        ...
        if total_puq_hits == 0:
            ...
            print(f"\nKILL: L3=0. puq_bfs+={total_puq_hits} but puq_lhs=0 - "
                  f"gic or tuv phase not completing.")
        else:
            print(f"\nKILL: L3=0. Reaches lhs ({total_puq_lhs}) but win not triggered. "
                  f"Check state ordering.")
    else:
        print(f"\nKILL: L3=0 and L2=0 - regression.")
