"""
step0974_chain_972_alpha_reset.py -- 972 chain + pred_errors reset on game switch.

FAMILY: Action-conditioned prediction chain diagnostic (one-fix from 973)
R3 HYPOTHESIS: Resetting _pred_errors deque on game switch prevents alpha contamination
from CIFAR's iid errors. LS20 should recover toward standalone (~72+ at 10K).

Root cause from Step 973: alpha_conc=1.4 in chain (vs 22-50 standalone). CIFAR's
uniform pred_errors fill the 200-slot deque → alpha_conc→1 → 972's novelty signal
becomes uniform across dims → navigation degrades.

One change from 973: self._pred_errors = deque(maxlen=200) added to set_game().
Same as Step 965's h-reset fix.
...
class Chain972Fixed:
    """973 chain + pred_errors reset on game switch (alpha decontamination)."""
    ...
    def __init__(self, seed):
        ...
        # Alpha: persistent across games (ENC_DIM, n_actions-independent)
    ...
    def set_game(self, n_actions):
        # Reset h (per 965 fix)
        self.h = np.zeros(H_DIM, dtype=np.float32)
        # Reset pred_errors to prevent alpha contamination (ONE-LINE FIX from 973)
        self._pred_errors = deque(maxlen=200)
        ...
        # Reset W_pred only if n_actions changed
...
if __name__ == "__main__":
    import os
    print("=" * 70)
    print("STEP 974 — 972 CHAIN + pred_errors RESET (alpha decontamination)")
    print("=" * 70)
    t0 = time.time()
    ...
    print(f"Fix: _pred_errors reset on set_game() (one-line, same as 965 h-reset fix)")
    print()
    ...
    print()
    print("=" * 70)
    print(f"STEP 974 RESULTS (baseline: 965@10K chain: LS20=67/6/10):")
