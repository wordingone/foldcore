"""
Step 618 — 572u base + 616 L0 fix + L3/L4 bootstrap chain. NO G resets.

Root cause of 617 failure: L0 cluster targeting freezes on animation artifact
  ayx=(57.39,5.39). visited accumulates all near targets → best=None → argmin=0
  forever. Done never fires. Episodes never complete after episode 2.

Root cause confirmed: 572u verbatim (617b) also fails (go=2, cyc=0 in 5min).

Fix: replace L0 cluster targeting with step 616's approach:
  1. Remove L0 cluster targeting (no more ayx dependency at L0)
  2. Add random tie-breaking to argmin (step 615/616 approach)
  Step 616 achieved go=7071, cyc=6965, L1 reached — confirmed L0 works without clustering.

Changes from 572u:
  - game_level=3 support (L4 tracking)
  - L3 bootstrap: _update_bg builds puq wall_set mid-run at game_level==2 [from 612]
  - L4 bootstrap: _update_bg builds l4_targets at game_level==3 [from 613]
  - on_l3(): now sets game_level=3
  - on_l4(): new handler
  - act(): L0 cluster targeting REMOVED; random tie-breaking added to argmin
  - NO G resets anywhere (confirmed correct from 572u)

2 seeds x 300s. Signal: L3 >= 1/2 (confirms 572u mgu/puq works). L4 >= 1/2 = win.
"""
import numpy as np
import time
import sys
...
class SubDual:
    def __init__(self, seed=0):
        ...
        self._puq_lhs_ap = 0
        self._puq_bootstrap_done = False
        self._l4_bootstrap_done = False
    ...
    def _update_bg(self, arr):
        ...
        if self.game_level == 0:
            ...
            if self.l3_frames % MODE_EVERY == 0:
                ...
                # L3 bootstrap: build puq wall_set mid-run (mirrors mgu bootstrap)
            ...
            if self.l4_frames % MODE_EVERY == 0:
                ...
                # L4 bootstrap: build cluster targets once enough L3 episodes seen
    ...
    def on_l2(self):
        ...
        if self._puq_wall_frozen:
            pass
        elif not self._puq_bootstrap_done and self.l2_count >= N_MAP_PUQ and self.l3_frames >= WARMUP:
            self._puq_wall_set = build_wall_set(self.l3_mode)
            self._puq_bootstrap_done = True
    ...
    def act(self):
        ...
        # L0 cluster targeting removed (ayx freeze caused visited saturation → argmin=0 forever)
        # Use random tie-breaking argmin instead (step 616 approach, confirmed go=7071 cyc=6965)
        counts = [sum(self.G.get((self._cn, a), {}).values()) for a in range(N_A)]
        min_c = min(counts)
        ties = [a for a, c in enumerate(counts) if c == min_c]
        action = int(self.rng.choice(ties))
...
def main():
    ...
    t_start = time.time()
    print("Step 618: LS20 L4 chain (572u + 616 L0 fix + L3/L4 bootstrap)", flush=True)
    print(f"  L0: random tie-breaking argmin (no cluster targeting — ayx freeze fix)", flush=True)
    print(f"  L1/L2: 572u mgu/puq (NO G resets)", flush=True)
    print(f"  Added: L3 bootstrap (puq wall_set mid-run), L4 bootstrap (cluster targets)", flush=True)
    print(f"  Signal: L3>=1/2 (572u mgu works). L4>=1/2 = win.", flush=True)
    ...
    for seed in range(n_seeds):
        ...
        for step in range(1, 1_000_001):
            ...
            if prev_cl == -1:
                if cl == 1:
                    ...
                    if l2_first[0] is None:
                        l2_first[0] = step
                        print(f"  s{seed} L2@{step} l2c={sub.l2_count} boot={sub._puq_bootstrap_done}", flush=True)
                elif cl == 3:
                    sub.on_l3(); l3_steps.append(step)
                    print(f"  s{seed} L3@{step}! l3c={sub.l3_count} l4boot={sub._l4_bootstrap_done}", flush=True)
                elif cl == 4:
            ...
            if time.time() - t_seed > per_seed_cap:
                print(f"  cap @{step} go={go} cyc={sub.l1_cycles} "
                      f"l2c={sub.l2_count} l3c={sub.l3_count} l4c={sub.l4_count} "
                      f"l3boot={sub._puq_bootstrap_done} l4boot={sub._l4_bootstrap_done} "
                      f"deaths={sub._puq_death_count} wp={sub._puq_wp_idx} lhs={sub._puq_lhs_ap}",
                      flush=True)
                break

        R.append({'seed': seed, 'l2c': sub.l2_count, 'l3c': sub.l3_count, 'l4c': sub.l4_count,
                  'l3_steps': l3_steps, 'l4_steps': l4_steps,
    ...
    print(f"\n{'='*60}", flush=True)
    print(f"Step 618: LS20 L4 chain (572u + 616 L0 fix)", flush=True)
    ...
    if l4_wins >= 1:
        ...
        print("  L3 preserved. L4 needs more bootstrap cycles or different L3 nav.", flush=True)
    else:
        print("  L3 not reached. mgu/puq failing in current env — investigate.", flush=True)
