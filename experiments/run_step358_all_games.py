#!/usr/bin/env python3
"""
Step 358 -- ARC-AGI-3: map ALL 3 games with pure argmin at 16x16.

Pure novelty-seeking (argmin), persistent codebook, sampled thresh.
10000 steps per game (LS20, FT09, VC33).
Script: scripts/run_step358_all_games.py
"""
...
def main():
    t0 = time.time()
    print("Step 358 -- map ALL 3 games: pure argmin, 16x16, 10000 steps each", flush=True)
    ...
    for g in games:
        print(f"  {g.title} ({g.game_id})", flush=True)
    print(flush=True)

    results = []
    for g in games:
        print(f"--- Running: {g.title} ({g.game_id}) ---", flush=True)
        r = run_game(arc, g.game_id, g.title, max_steps=10000, max_resets=200, k=3)
        results.append(r)
        print(f"    Done: levels={r['levels']} cb={r['cb_final']} unique={r['unique_states']}"
              f" go={r['game_over']} frame_change_rate={r['frame_change_rate']:.2%}", flush=True)
        print(flush=True)

    elapsed = time.time() - t0

    print("=" * 60, flush=True)
    print("STEP 358 SUMMARY — ALL GAMES", flush=True)
    print("=" * 60, flush=True)
    for r in results:
        print(flush=True)
        print(f"  {r['title']} ({r['game_id']}):", flush=True)
