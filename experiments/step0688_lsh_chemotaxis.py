"""
Step 688 — Chemotaxis-inspired biased random walk for L2.

674 substrate (transition-triggered dual-hash, k=12+k=20) +
biased random walk based on new-cell discovery rate.

Sliding window (500 steps): 1 if new cell discovered, 0 otherwise.
Derivative = (last 250) - (first 250) within window.
  if derivative > 0: argmin (extend run — discovery increasing)
  if derivative <= 0: random 0.3, argmin 0.7 (partial tumble)

Seed 8, 300s cap. Report every 30s: mode_switches, discovery_rate,
aliased_cells, L2 status.
"""
import numpy as np
import time
import sys
...
def main():
    ...
    print(f"Step 688: Chemotaxis on seed {SEED}, {PER_SEED_TIME}s cap")
    rng = np.random.RandomState(SEED * 999 + 1)
    ...
    # Chemotaxis state
    discovery_window = deque(maxlen=WINDOW_SIZE)
    seen_nodes = set()
    mode_switches = 0
    tumble_steps = 0
    run_steps = 0
    ...
    for step in range(1, MAX_STEPS):
        ...
        # Track new cell discovery
        is_new = 1 if node not in seen_nodes else 0
        seen_nodes.add(node)
        discovery_window.append(is_new)

        # Chemotaxis mode selection
        if len(discovery_window) >= WINDOW_SIZE:
            prev_half = sum(list(discovery_window)[:HALF])
            recent_half = sum(list(discovery_window)[HALF:])
            derivative = recent_half - prev_half
            if derivative <= 0:
                # Partial tumble: 30% random, 70% argmin
                if rng.random() < TUMBLE_PROB:
                    action = int(rng.randint(0, N_A))
                    if not in_tumble:
                        mode_switches += 1; in_tumble = True
                else:
                    action = base_action
                tumble_steps += 1
            else:
            ...
            # Insufficient window: use argmin
            action = base_action
            run_steps += 1
        ...
        if elapsed >= next_report:
            disc_rate = sum(discovery_window) / len(discovery_window) if discovery_window else 0.0
            total = run_steps + tumble_steps
            tumble_frac = tumble_steps / total if total > 0 else 0.0
            print(f"  t={elapsed:.0f}s step={step} aliased={len(sub.aliased)} "
                  f"disc_rate={disc_rate:.3f} tumble_frac={tumble_frac:.2f} "
                  f"mode_sw={mode_switches} L1={l1} L2={l2}", flush=True)
    ...
    print(f"total_seen_nodes={len(seen_nodes)}  mode_switches={mode_switches}")
    if l2: print("BREAKTHROUGH: L2 reached!")
    elif l1: print("FINDING: L1 reached, L2 not — chemotaxis didn't unlock L2")
