"""
step1001_oscillatory.py -- Stuart-Landau oscillatory substrate.

FAMILY: Oscillatory dynamics (new family — not 916, not Hebbian, not graph)
R3 HYPOTHESIS: Phase-coherence gating provides temporal credit for sequential
actions without per-state storage (addresses Theorem 4). The substrate
self-modifies WHICH connections grow (phase-coherence eligibility) and WHEN
credit flows (compression-progress modulator). Action selection, coupling
weights, and prediction all run on one signal: compression progress M(t).

916 ceiling: LS20=286.2/9/10 @25K, FT09=0, VC33=0 — blocked on temporal
credit assignment. Oscillatory phase coherence is a physically-grounded
temporal credit mechanism with no per-state storage.

Step 1001: LS20 diagnostic only. Does it navigate AT ALL?
Kill: LS20 < 20 at 10K. Success: any seed L1 > 0.
...
class Oscillatory1001:
    ...
    def __init__(self, seed):
        ...
        # Modulator state: M(t) = max(0, err_ema_slow - err_ema_fast)
        self._err_ema_slow = 1.0
        self._err_ema_fast = 1.0
    ...
    def set_game(self, n_actions):
        ...
        # Fresh oscillator phases for new game context
    ...
    def process(self, obs):
        ...
        # Modulator + learning updates (require previous step)
        M = 0.0
        if self._prev_features is not None:
            # W_pred: predict current enc from previous features
            pred_enc = self._W_pred @ self._prev_features
            error = enc - pred_enc
            err_scalar = float(np.linalg.norm(error))

            # Modulator EMAs
            self._err_ema_slow = ERR_EMA_SLOW * self._err_ema_slow + (1 - ERR_EMA_SLOW) * err_scalar
            self._err_ema_fast = ERR_EMA_FAST * self._err_ema_fast + (1 - ERR_EMA_FAST) * err_scalar
            M = max(0.0, self._err_ema_slow - self._err_ema_fast)

            # W_pred update: gradient DESCENT (+ sign) — NOT the same as 916.
            # 916 uses gradient ascent (- sign) to drive alpha calibration.
            # Here W_pred must actually improve to make M(t) fire.
            # Gradient ascent keeps fast_EMA > slow_EMA → M=0 always (verified: 8/500 steps).
            # Gradient descent keeps errors bounded and M fires ~52% of steps.
    ...
    def phase_diversity(self):
        """Std of oscillator phases — diagnostic for whether phases spread."""
        phi = np.angle(self._z)
        return float(np.std(phi))

    def modulator_state(self):
        """Current M(t) and EMA values — diagnostic."""
        M = max(0.0, self._err_ema_slow - self._err_ema_fast)
        return M, self._err_ema_slow, self._err_ema_fast
...
if __name__ == "__main__":
    import os
    print("=" * 70)
    print("STEP 1001 — OSCILLATORY SUBSTRATE (Stuart-Landau, phase-coherence credit)")
    print("=" * 70)
    ...
    print(f"Modulator: err_ema_slow={ERR_EMA_SLOW}, err_ema_fast={ERR_EMA_FAST}")
