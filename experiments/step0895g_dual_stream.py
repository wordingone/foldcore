...
zc = sum(1 for x in cold_comps if x == 0); zw = sum(1 for x in warm_comps if x == 0)
...
print(f"\ncold: L1={mc:.1f}/seed  std={sc:.1f}  zero-seeds={zc}/{len(cold_comps)}  alpha_conc={np.mean(cold_concs):.2f}  pred_acc={mcp:.3f}")
print(f"      {cold_comps}")
print(f"warm: L1={mw:.1f}/seed  std={sw:.1f}  zero-seeds={zw}/{len(warm_comps)}  alpha_conc={np.mean(warm_concs):.2f}  pred_acc={mwp:.3f}")
print(f"      {warm_comps}")
print(f"L1 delta (warm-cold): {mw-mc:+.1f}/seed  std reduction: {sc/sw:.1f}x (cold/warm)")
print(f"\nVariance comparison (key metric per Leo mail 2626):")
print(f"  868b plain 800b (25K): mean=72.1 std=112 zero=5/10")
print(f"  895c warm (alpha-delta, 10K): mean=77.8 std=35 zero=0/5")
print(f"  895g warm (dual-stream, 25K): mean={mw:.1f} std={sw:.1f} zero={zw}/{len(warm_comps)}")
print(f"\nBaselines:")
