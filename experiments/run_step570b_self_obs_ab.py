"""
Step 570b — Self-observation via graph planning: LS20 A/B test.

Fix: _seek() uses plain argmax successor (no >0.5 determinism threshold).
Applied in BOTH the BFS loop AND the greedy extension loop.

A: Sub(self_obs=False) — pure argmin baseline
B: Sub(self_obs=True)  — BFS routing after exploration stall (no new nodes 500 steps)

10 seeds x 20K steps each condition. 5-min cap total.
Kill: if B still plans=0 or B <= A at 20K.
"""
import numpy as np
