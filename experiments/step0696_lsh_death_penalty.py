"""
Step 696 — 674 + death penalty (from 581d).

674 (transition-triggered dual-hash) + soft death penalty:
when agent dies at (cell, action), add DEATH_BONUS to that edge's score.
Action selection: score(a) = count(a) + DEATH_BONUS * death_count[(cell, a)]
action = argmin(score) — avoids historically lethal actions.

581d alone: 4/5 L1. 674 alone: 9/10. Combined: does death penalty
help slow seeds (s0, s4) while 674 handles perception?

10 seeds (0-9), 25s cap.
Kill: L1 < 9/10 or avg speed worse than 674 alone.
"""
import numpy as np
import time
import sys
...
N_SEEDS = 10
MIN_VISITS_ALIAS = 3
DEATH_BONUS = 1000  # large positive added to score of death edges
...
class TransitionTriggeredDeath:
    ...
    def act(self):
        if self._cn in self.aliased and self._fn is not None:
            ...
            for a in range(N_A):
                ...
                score = count + DEATH_BONUS * deaths
...
def run(seed, make):
    env = make()
    sub = TransitionTriggeredDeath(seed=seed * 1000)
    ...
    if l1 and bsl:
        ...
        spd = "NO_L1"
    print(f"  s{seed:2d}: L1={l1}({spd}) L2={l2} aliased={len(sub.aliased)} deaths={sum(sub.death_edges.values())}", flush=True)
...
def main():
    ...
    print(f"Step 696: 674+death_penalty(bonus={DEATH_BONUS}) on LS20, {N_SEEDS} seeds, {PER_SEED_TIME}s cap")
    ...
    if l1_n >= 10: print("FINDING: 674+death=10/10 — death penalty fixes regressions")
    elif l1_n >= 9: print("FINDING: Matches 674 baseline — death penalty neutral")
    else: print(f"KILL: L1={l1_n}/{N_SEEDS} — death penalty hurts")
