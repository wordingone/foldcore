"""
Step 701 — Plain k=12 argmin baseline, 20 seeds, 120K steps.

Step 697 showed plain k=12 = 11/20 at 25s.
Does plain k=12 reach 20/20 at 120K? If yes: 674 is speed improvement only.
If no: 674 provides coverage improvement too.
"""
import numpy as np
import time
import sys
...
def main():
    ...
    t_start = time.time()
    print(f"Step 701: Plain k=12 baseline, {N_SEEDS} seeds, {MAX_STEPS-1} steps cap")
    print(f"Game: LS20 (current version — check init log for hash)")
    ...
    print(f"BASELINE: Plain k=12 at 120K = {l1_n}/{N_SEEDS}")
    if l1_n >= 20:
        print("NOTE: 674 is speed improvement only — plain k=12 also reaches 20/20 at 120K")
    else:
        print(f"NOTE: 674 provides coverage improvement — plain k=12 misses {20-l1_n} seeds even at 120K")
