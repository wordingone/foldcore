"""
Step 716b — Outcome-based action pruning, k_prune=8, full run on LS20.

R3 hypothesis: graph-derived pruning (k_prune=8) correctly identifies dir actions
as STRUCTURAL and click actions as COSMETIC on LS20 at full budget.

Expected: structural_dir >= 2, structural_click = 0 at end of 120K steps.

Game: ls20/9607627b. 5 seeds, 120K steps.
"""
import numpy as np
import time
import sys
...
def main():
    ...
    t_start = time.time()
    print(f"Step 716b: Outcome-based AD (k_prune=8) on LS20, {N_SEEDS} seeds, {MAX_STEPS-1} steps")
    print(f"Expected: struct_dir>=2, struct_click=0 (dirs structural, clicks cosmetic)")
    results = []
    ...
    correct = sum(1 for r in results if r['struct_dir'] > 0 and r['struct_click'] == 0)
    print(f"\nR3: correct discovery (dirs structural, clicks cosmetic) in {correct}/5 seeds")
    if correct >= 3:
        print(f"PASS: LS20 action discovery correct with k_prune=8")
    else:
        print(f"FAIL: LS20 discovery incorrect at k_prune=8")
