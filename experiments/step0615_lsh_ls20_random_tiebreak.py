"""
Step 615: LS20 fix — random tie-breaking in argmin + no G reset.

Root cause found in 614: go=2 in 114K steps (only 2 game episodes in 5 min).
  - LS20 has ~130-movement episode limit. done fires after 130 real movements.
  - With argmin([0,0,0,0]) = 0, agent ALWAYS takes action=0 (up) on fresh nodes.
  - Agent spawns at top wall → action=0 hits wall → no movement → counter never
    reaches 130 → done never fires → 55K steps, go=2.
  - probe confirms: constant action=0 → 0 done in 5000 steps.
    Random actions → done fires every ~130 steps (3 in 500 steps).

Fix: Random tie-breaking in argmin.
  When multiple actions have equal minimum counts, choose randomly among them.
  Instead of always picking action=0 on fresh nodes, agent explores all 4 directions.
  ~3/4 of actions become valid movements → done fires every ~170 steps → L1 reachable.

Also keeps:
  - No G reset on level transitions (614 fix)
  - L3/L4 bootstrap (Option C) from 612/613

2 seeds × 5 min. Signal: L2 >= 1/2. Kill: L2 = 0/2.
"""
import numpy as np
import time
import sys
...
def main():
    ...
    t_start = time.time()
    print("Step 615: LS20 random tie-breaking in argmin + no-G-reset", flush=True)
    print(f"  Fix: random choice among min-count actions (was always action=0)", flush=True)
    print(f"  Also: no G reset on transitions, L3/L4 bootstrap", flush=True)
    ...
    print(f"\n{'='*60}", flush=True)
    print(f"Step 615: LS20 random-tiebreak", flush=True)
    ...
    if l4_wins >= 1:
        ...
        print("  L2 reached. Random tie-break + no-G-reset confirmed. L3 bootstrap pending.", flush=True)
    else:
        print("  FAIL: L2 not reached. Check go count and episode cycling.", flush=True)
