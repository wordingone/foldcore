"""
step0975_chain_972_fixed_actions.py -- 972 chain with fixed MAX_ACTIONS for W_pred.

FAMILY: Action-conditioned prediction chain diagnostic (W_pred persistence fix)
R3 HYPOTHESIS: Fixed MAX_ACTIONS=100 makes W_pred dimension-independent across games.
W_pred(256, 64+320+100) never resets between games — LS20 inherits W_pred from CIFAR.
This restores the persistence that made 965 work (67/6/10 vs 973's 38.6).

Root cause from 973-974: W_pred reset on n_actions change (CIFAR 100→LS20 4)
forced LS20 to cold-start W_pred=0. 916's W_pred(320,320) is n_actions-independent
→ persists → LS20 inherits warm W_pred → 67.0 in 965.

One change from 974: W_pred has fixed (256, 64+320+100=484) dimensions. one_hot
padded to MAX_ACTIONS=100. W_pred NEVER resets. h resets on game switch (per 965).
...
MAX_ACTIONS = 100            # Fixed action space for W_pred persistence
PRED_INPUT_DIM = H_DIM + EXT_DIM + MAX_ACTIONS  # 484
ETA_W = 0.01
ALPHA_EMA = 0.10
INIT_DELTA = 1.0
...
class Chain972MaxActions:
    """972 chain with fixed MAX_ACTIONS W_pred — persists across all games."""
    ...
    def __init__(self, seed):
        ...
        # W_pred: fixed size, NEVER resets between games
    ...
    def set_game(self, n_actions):
        ...
        self._prev_ext = None
        self._prev_action = None
        # W_pred NEVER resets (fixed MAX_ACTIONS dimension)
    ...
    def process(self, obs):
        ...
        if self._prev_h is not None and self._prev_action is not None:
            # Pad one_hot to MAX_ACTIONS
            one_hot_a = np.zeros(MAX_ACTIONS, dtype=np.float32)
            one_hot_a[self._prev_action] = 1.0  # action < MAX_ACTIONS always
...
def run_chain(seeds, n_steps, cifar_steps, cifar_imgs, cifar_lbls):
    ...
    for seed in seeds:
        sub = Chain972MaxActions(seed=seed)
...
if __name__ == "__main__":
    import os
    print("=" * 70)
    print("STEP 975 — 972 CHAIN FIXED MAX_ACTIONS (W_pred persistent)")
    print("=" * 70)
    t0 = time.time()
    ...
    print(f"W_pred: (256, {PRED_INPUT_DIM}) fixed — one_hot padded to MAX_ACTIONS={MAX_ACTIONS}")
    print(f"W_pred NEVER resets. h resets on game switch. alpha/enc_mean persist.")
    print()
    ...
    print()
    print("=" * 70)
    print(f"STEP 975 RESULTS (965@10K: LS20=67/6/10, 973 chain: LS20=38.6/8/10):")
    ...
    ls20_baseline = 67.0  # 965 chain baseline with h-reset
    ...
    print(f"  Chain kill check (vs 965): {chain_verdict}")
