"""
Step 572d — Diagnostic: hardcoded lhs target in mgu.

572c root cause: lhs (color 5, 5x5px) in mgu not in l2_targets because
color 5 is used by HUD/snw sprite → not rare → never targeted → L2 impossible.

Fix: after N_MAP cycles, append hardcoded lhs target at (cy=42, cx=16)
to l2_targets (mgu's lhs at set_position(14,40) = col=14, row=40 → cy=40+2=42, cx=14+2=16 center).

Keeps all other 572c architecture (prev_cl tracking, no visited in L2 phase).
If L2 > 0: HUD masking (572e) is the R1-compliant fix.
"""
import numpy as np
import time
import sys
...
# Hardcoded mgu (Level 1) lhs position from game source:
# set_position(col=14, row=40), 5x5 sprite → center cy=42, cx=16
LHS_HARDCODE = {'cy': 42.0, 'cx': 16.0, 'color': 5, 'size': 25, 'hardcoded': True}
...
class SubDual:
    def __init__(self, seed=0):
        ...
        self.lhs_visits = 0  # times agent reached lhs area
    ...
    def on_l1(self):
        ...
        if self.l1_cycles >= N_MAP and self.l2_frames >= WARMUP:
            ...
            # Hardcode lhs — not detectable via mode map (HUD masking)
            self.l2_targets.append(dict(LHS_HARDCODE))
        ...
        if targets and self.agent_yx is not None:
            ...
            if best is not None:
                if best_dist < VISIT_DIST:
                    if self.game_level == 1 and best.get('hardcoded'):
                        self.lhs_visits += 1
                    if self.game_level == 0:
...
def main():
    ...
    for seed in range(n_seeds):
            ...
            if cl > prev_cl:
                ...
                if cl == 2 and l2_step is None:
                    ...
                    print(f"  s{seed} L2@{step}! cycle={sub.l1_cycles} "
                          f"lhs_visits={sub.lhs_visits} go={go}", flush=True)
            prev_cl = cl

            if step % 25_000 == 0:
                el = time.time() - seed_start
                print(f"  s{seed} @{step} cycles={sub.l1_cycles} l2f={sub.l2_frames} "
                      f"l2tgt={sub.n_l2_tgt} lhs_v={sub.lhs_visits} go={go} {el:.0f}s", flush=True)
        ...
        R.append(dict(seed=seed, l1=l1_step, l2=l2_step, go=go, steps=step,
                      ...
                      n_l2_tgt=sub.n_l2_tgt, lhs_visits=sub.lhs_visits,
    ...
    for r in R:
        ...
        print(f"  s{r['seed']}: {tag}  cycles={r['cycles']}  l2f={r['l2_frames']}  "
              f"l2tgt={r['n_l2_tgt']}  lhs_v={r['lhs_visits']}  go={r['go']}  ta={r['ta']}")
    l1n = sum(1 for r in R if r['l1']); l2n = sum(1 for r in R if r['l2'])
    print(f"\nL1={l1n}/{len(R)}  L2={l2n}/{len(R)}")
    if l2n > 0: print(f"\nFIND: L2={l2n}/{len(R)}! Hardcoded lhs works — HUD masking is the fix.")
    else: print(f"\nKILL: L2=0. lhs_visits={[r['lhs_visits'] for r in R]}. "
                f"Agent reaches lhs area but state puzzle fails, or wrong position.")
