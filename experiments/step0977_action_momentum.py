"""
step0977_action_momentum.py -- Action momentum: repeat high-delta actions.

FAMILY: Sequential momentum (916 base with h-reset from 965)
R3 HYPOTHESIS: When delta > MOMENTUM_THRESHOLD, repeat prev_action for MOMENTUM_STEPS
more steps. High delta = something interesting happened — repeating from the new state
may trigger the next step of a sequential puzzle (FT09's 7-step sequence). "Exploit
what just worked" without per-state memory.

Ban-safe: momentum = global scalar counter + global action. No (state,action) tracking.
One config universal (MOMENTUM_THRESHOLD=2.0, MOMENTUM_STEPS=3).

Kill: LS20 < 67.0 (chain baseline) OR FT09 = 0/10.
Success: FT09 > 0 (momentum discovers sequences). LS20 holds.

Changes from 965: process() checks delta > threshold → set momentum. Action selection
uses momentum_action if momentum > 0, else standard 800b softmax.
...
EPSILON = 0.20
SOFTMAX_TEMP = 0.10
MOMENTUM_THRESHOLD = 2.0  # delta > this triggers momentum
MOMENTUM_STEPS = 3        # repeat high-delta action this many more times
...
class ActionMomentum977:
    """916 base + action momentum. Repeat high-delta action for K steps."""
    ...
    def process(self, obs):
        ...
        if self._prev_ext is not None and self._prev_action is not None:
            ...
            # Momentum: high delta → repeat this action
            if delta > MOMENTUM_THRESHOLD:
                self._momentum = MOMENTUM_STEPS
                self._momentum_action = a

        # Action selection: momentum overrides standard selection
        if self._momentum > 0:
            action = self._momentum_action
            self._momentum -= 1
...
def run_chain(seeds, n_steps, cifar_steps, cifar_imgs, cifar_lbls):
    ...
    for seed in seeds:
        sub = ActionMomentum977(seed=seed)
...
if __name__ == "__main__":
    import os
    print("=" * 70)
    print("STEP 977 — ACTION MOMENTUM (repeat high-delta action)")
    print("=" * 70)
    t0 = time.time()
    ...
    print(f"Momentum: threshold={MOMENTUM_THRESHOLD}  steps={MOMENTUM_STEPS}")
    print(f"916 base + h-reset. W_pred/alpha/enc_mean persist. running_mean resets.")
    print()
    ...
    print()
    print("=" * 70)
    print(f"STEP 977 RESULTS (965 chain: LS20=67.0, standalone 916@10K=74.7):")
