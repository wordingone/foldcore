"""
Step 716 — Outcome-based action pruning, k_prune=8, full run on VC33.

R3 hypothesis: graph-derived action pruning at k_prune=8 discovers VC33's magic
clicks as STRUCTURAL actions from transition topology alone.

Mechanism: 674+running-mean nav (k_nav=12, k_fine=20), PLUS separate k_prune=8
raw uncentered hash for cosmetic/structural classification. Min_probes=5.
Revival: cosmetic action that leads to new prune cell -> revive.

Track: UNIV[35] (action 1852) and UNIV[43] (action 2364) structural at end?

Game: vc33/9851e02b. 5 seeds, 120K steps.
"""
import numpy as np
import time
import sys
...
class OutcomeAD_k8:
    def __init__(self, seed=0):
        ...
        # Outcome-based pruning state (persists across episodes)
        self.structural = set(range(N_UNIV))
        self.cosmetic = set()
        self.new_cell_count = [0] * N_UNIV
        self.total_count = [0] * N_UNIV
        # Episode-local novelty (resets with _mu)
    ...
    def peek_prune(self, x_raw):
        """Check if raw frame is novel in k_prune space. Does not update state."""
        ph = _hash_k(self.H_prune, x_raw)
        return ph not in self.all_seen_prune, ph
    ...
    def report_outcome(self, action_idx, obs_curr, obs_new):
        """Outcome: did this action lead to a novel prune cell?"""
        # Add current obs to all_seen before checking result
    ...
    def on_reset(self):
        ...
        self.all_seen_prune = set()  # episode-local
...
def run(seed, make):
    ...
    cosm = len(sub.cosmetic)
    magic_a = MAGIC_UNIV_IDX_A in sub.structural
    magic_b = MAGIC_UNIV_IDX_B in sub.structural
    struct_list = sorted(sub.structural)
    ...
    print(f"  s{seed:2d}: {bootloader} structural={len(sub.structural)}(dir={struct_dir},click={struct_click}) "
          f"cosmetic={cosm} magic=({'Y' if magic_a else 'N'},{'Y' if magic_b else 'N'}) "
          f"aliased={len(sub.aliased)} t={elapsed:.1f}s", flush=True)
    print(f"         structural_actions={struct_list}", flush=True)
    return dict(seed=seed, l1=l1, structural=len(sub.structural), cosmetic=cosm,
                struct_dir=struct_dir, struct_click=struct_click,
                magic_a=magic_a, magic_b=magic_b, struct_list=struct_list)
...
def main():
    ...
    t_start = time.time()
    print(f"Step 716: Outcome-based AD (k_prune=8) on VC33, {N_SEEDS} seeds, {MAX_STEPS-1} steps")
    print(f"R3: graph-derived pruning, k_prune=8 raw hash. Magic: UNIV[{MAGIC_UNIV_IDX_A}], UNIV[{MAGIC_UNIV_IDX_B}]")
    results = []
    ...
    magic_n = sum(1 for r in results if r['magic_a'] or r['magic_b'])
    magic_both = sum(1 for r in results if r['magic_a'] and r['magic_b'])
    print(f"\nR3 result:")
    print(f"  Magic detected: {magic_both}/5 both, {magic_n}/5 either")
    if boot_n >= 3 and magic_both >= 3:
        print(f"SIGNAL: bootloader {boot_n}/5 + magic both structural {magic_both}/5 — R3 confirmed, proceed to 717 chain")
    elif magic_both >= 3:
        print(f"PARTIAL: magic structural {magic_both}/5 but bootloader fails — pruning works, nav broken")
    elif magic_n >= 3:
        print(f"PARTIAL: one magic click structural {magic_n}/5 but not both")
    else:
        print(f"KILL: magic clicks not reliably structural ({magic_n}/5)")
