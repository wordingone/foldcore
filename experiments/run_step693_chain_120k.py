"""
Step 693 — Chain benchmark with 674 at 120K steps (definitive).

Chain: CIFAR-100 P1 (1K obs) -> LS20 (120K steps) -> CIFAR-100 P3 (1K obs).
Per-domain centering (on_reset resets running mean).
674 substrate: k=12 nav + k=20 fine at aliased cells. Binary aliasing.

Step 687 (300s cap): 3/3 L1, CIFAR acc=1.4%, 8-11 aliased during CIFAR.
This step: 5 seeds, 120K steps LS20 phase.

Kill: L1 < 3/5 or CIFAR contamination > 5pp.
"""
import numpy as np
import time
import sys
...
def main():
    ...
    print(f"Step 693: Chain with 674 substrate. {CIFAR_N} CIFAR obs per phase, {LS20_STEPS} LS20 steps.")
    R = []
    for seed in range(5):
    ...
    print(f"CIFAR-P1 contamination: {cifar1_contamination} (Step 687 was [10,11,8])")
    if l1n >= 3: print("FINDING: 674 chain compatible at 120K steps")
    else: print("KILL: Chain disrupts 674 at 120K steps")
