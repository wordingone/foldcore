"""
step0938e_916_plus_h_perturbation.py -- Full 916 delta + h perturbation.

R3 hypothesis: 916's delta (temporal: which action caused most change historically)
combined with R@h (reactive: trajectory-dependent action bias) provides complementary
position-dependent signal. Delta is position-blind (Prop 23b). R@h breaks ties
between equal-delta actions with current trajectory state.

Architecture: Full 916 (alpha-weighted ext_enc delta, h in delta computation).
ONLY addition: score = delta_ema + beta * |R@h|.

delta_ema[a] = 0.9*delta[a] + 0.1*||alpha*(ext_enc_t - ext_enc_{t-1})|| (same as 916)
h_bias = |R @ h|  (R fixed seed=42, shape n_actions×H_DIM)
score = delta_ema + beta * h_bias
action = softmax(score/T) with epsilon=0.20

Small beta: [0.01, 0.05, 0.1]. Testing whether ANY h-perturbation signal exists.
Kill: LS20 < 268 at any beta (perturbation hurts 916 baseline).
Signal: FT09 > 0 (first non-zero post-ban = major finding).
...
class Chain938e:
    """Full 916 delta on alpha-weighted ext_enc (h included) + small beta * |R@h|.

    Identical to 916 except: score = delta_ema + beta * |R@h|.
    Alpha + running_mean persist. W_pred + h + delta reset per game.
    """

    def __init__(self, seed=0, beta=0.05):
    ...
    def process(self, obs):
        ...
        if self._prev_ext is not None and self._prev_action is not None:
            ...
            # Delta: full 916 (alpha-weighted ext_enc, h included)
        ...
        # h perturbation: small additive bias
        h_bias = np.abs(self.R @ self.h)   # (n_actions,)

        # 916 delta + beta * h perturbation
        ...
        if self._phase_step in DIAG_STEPS:
            self._diag_log[self._phase_step] = {
                ...
                "h_contribution": float(self.beta * h_bias.max()),
            }
    ...
    def diag_str(self):
        ...
        for step in sorted(self._diag_log):
            ...
            parts.append(f"@{step//1000}K: alpha_conc={d['alpha_conc']:.2f}"
                         ...
                         f" h_contrib={d['h_contribution']:.3f}"
...
print("=" * 70)
print("STEP 938e — 916 DELTA + SMALL h PERTURBATION")
print("=" * 70)
print("916 delta (alpha*ext_enc, h included) + beta*|R@h|. Small beta sweep.")
print(f"Betas: {BETAS}. 2 seeds each.")
t0 = time.time()
...
for beta in BETAS:
    ...
    for seed in TEST_SEEDS:
        sub = Chain938e(seed=seed, beta=beta)
        sub.reset_seed(seed)
        print(f"\n  Seed {seed}:")
...
print(f"\n{'=' * 70}")
print(f"STEP 938e RESULTS (916+h perturbation, PRISM-light, 10K/phase, {len(TEST_SEEDS)} seeds):")
print(f"{'Beta':>6}  {'LS20':>10}  {'FT09':>8}  {'VC33':>8}  CIFAR-1")
for beta in BETAS:
    r = all_results[beta]
    print(f"  {beta:>5}:  LS20={np.mean(r['ls20']):6.1f}/seed"
...
print(f"\nKill: LS20 < 268 at any beta (h hurts). Signal: FT09 > 0.")
print(f"Total elapsed: {time.time() - t0:.1f}s")
print("STEP 938e DONE")
