"""
Step 681 — Transition-triggered dual-hash on VC33.

Same as Step 680 (FT09) but for VC33. Click-based game.
k=12 coarse + k=20 fine at aliased cells. Binary aliasing criterion.
8x8 click grid (64 actions). 5 seeds, 25s cap.

Report: L1, L2, aliased_cells per seed.
"""
import numpy as np
import time
import sys
...
def run(seed, make):
    env = make()
    sub = TransitionTriggeredVC33(seed=seed * 1000)
...
def main():
    ...
    print(f"Step 681: Transition-triggered dual-hash on VC33 (8x8 grid, {N_A} actions): "
    ...
    if l2_n > 0: print("FINDING: L2 reached on VC33")
    elif l1_n >= 4: print("FINDING: L1 generalizes to VC33")
    elif l1_n >= 2: print("MARGINAL: Some L1 on VC33")
    else: print("KILL: Transition-triggered fails on VC33")
