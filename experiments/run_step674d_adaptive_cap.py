"""
Step 674d — Adaptive cap transition-triggered refinement (top 25% by inconsistency).

Same as 674, but instead of top-N, use a percentile threshold:
Only cells in the top 25% by inconsistency score become aliased.
Adapts to the actual distribution per seed.
...
N_SEEDS = 10
MIN_VISITS_ALIAS = 3
PERCENTILE_THRESHOLD = 75  # top 25% = above 75th percentile
...
class AdaptiveCappedTransitionTriggered:
    ...
    def _recompute_aliased(self):
        ...
        if not scores:
            self.aliased = set()
            return
        # Keep top 25% by inconsistency score (above 75th percentile)
        score_values = list(scores.values())
        threshold = float(np.percentile(score_values, PERCENTILE_THRESHOLD))
        self.aliased = {cell for cell, s in scores.items() if s >= threshold}
...
def run(seed, make):
    env = make()
    sub = AdaptiveCappedTransitionTriggered(seed=seed * 1000)
...
def main():
    ...
    print(f"Adaptive capped transition-triggered (top-25%): {N_SEEDS} seeds, {PER_SEED_TIME}s cap")
    ...
    if l2_n > 0:
        ...
        print("STRONG FINDING: Adaptive capped aliasing detection works")
    elif l1_n < 3:
        print("KILL: Adaptive capped transition-triggered fails")
    else:
