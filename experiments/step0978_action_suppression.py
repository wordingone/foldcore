"""
step0978_action_suppression.py -- Temporary action suppression after high delta.

FAMILY: Sequential momentum — anti-momentum variant (916 base, h-reset)
R3 HYPOTHESIS: When delta > SUPPRESS_THRESHOLD, suppress prev_action for SUPPRESS_STEPS.
Forces action VARIETY: after discovering something (high delta), the substrate must try
something DIFFERENT from the new state. Sequential puzzles require A→B→C, not A→A→A.
Anti-momentum: exploit what just worked, then move on.

Ban-safe: suppress[] is per-action counter (same structure as running_mean). Not
per-(state,action). Suppression is a global per-action cooldown.

Kill: LS20 < 67.0 (chain baseline).
Success: FT09 > 0 (suppression discovers sequential variety).

Changes from 965: suppress[n_actions] vector decays each step. High delta → suppress
prev_action for SUPPRESS_STEPS. Softmax excludes suppressed actions.
...
EPSILON = 0.20
SOFTMAX_TEMP = 0.10
SUPPRESS_THRESHOLD = 3.0  # delta > this suppresses prev_action
SUPPRESS_STEPS = 5        # suppress for this many steps
TEST_SEEDS = list(range(1, 11))
PHASE_STEPS = 10_000
CIFAR_STEPS = 1_000


def softmax_sel_masked(delta, suppress, temp, rng):
    """Softmax selection with suppressed actions excluded. Falls back if all suppressed."""
    x = np.array(delta) / temp
    x -= np.max(x)
    # Set suppressed actions to -1e9 before exp (probability → 0)
    for i, s in enumerate(suppress):
        if s > 0:
            x[i] = -1e9
    e = np.exp(x)
    total = e.sum()
    if total < 1e-10:
        # All suppressed or all -inf: ignore suppression, use raw delta
        x2 = np.array(delta) / temp
        x2 -= np.max(x2)
        e = np.exp(x2)
        total = e.sum()
    probs = e / (total + 1e-12)
    # Ensure valid probability distribution (handle any residual numerical issues)
    probs = np.clip(probs, 0, None)
    probs /= probs.sum()
    return int(rng.choice(len(delta), p=probs))


class ActionSuppression978:
    """916 base + temporary suppression after high delta. Forces action variety."""

    def __init__(self, seed):
        self._rng = np.random.RandomState(seed)
        rs = np.random.RandomState(seed + 10000)

        # Fixed random reservoir (same as 916/965)
        self.W_h = rs.randn(H_DIM, H_DIM).astype(np.float32) * 0.1
        self.W_x = rs.randn(H_DIM, ENC_DIM).astype(np.float32) * 0.1

        # W_pred: persists across games (same as 965)
        ...
        self.suppress = None  # per-action cooldown counter
    ...
    def set_game(self, n_actions):
        ...
        self.suppress = np.zeros(n_actions, dtype=np.int32)
        self._prev_ext = None
        self._prev_action = None
    ...
    def process(self, obs):
        ...
        # Decay all suppressions by 1 each step
        if self.suppress is not None:
            self.suppress = np.maximum(self.suppress - 1, 0)
        ...
        if self._prev_ext is not None and self._prev_action is not None:
            ...
            # Suppress action that caused high delta → force variety
            if delta > SUPPRESS_THRESHOLD:
                self.suppress[a] = SUPPRESS_STEPS

        # Action selection: exclude suppressed actions from softmax
        if self._rng.random() < EPSILON:
            # During epsilon: also respect suppression
            available = [i for i in range(self._n_actions) if self.suppress[i] == 0]
            if not available:
                available = list(range(self._n_actions))  # all suppressed → ignore suppression
            action = int(self._rng.choice(available))
        else:
            action = softmax_sel_masked(self.running_mean, self.suppress, SOFTMAX_TEMP, self._rng)
    ...
    def on_level_transition(self):
        ...
        self._prev_action = None
        if self.suppress is not None:
            self.suppress[:] = 0  # clear suppression on level reset

    def alpha_conc(self):
        return float(np.max(self.alpha) / (np.min(self.alpha) + 1e-8))

    def suppress_active(self):
        if self.suppress is None:
            return 0
        return int(np.sum(self.suppress > 0))
...
def run_chain(seeds, n_steps, cifar_steps, cifar_imgs, cifar_lbls):
    ...
    for seed in seeds:
        sub = ActionSuppression978(seed=seed)
...
if __name__ == "__main__":
    import os
    print("=" * 70)
    print("STEP 978 — ACTION SUPPRESSION (force variety after high delta)")
    print("=" * 70)
    t0 = time.time()
    ...
    print(f"Suppression: threshold={SUPPRESS_THRESHOLD}  steps={SUPPRESS_STEPS}")
    print(f"916 base + h-reset. W_pred/alpha/enc_mean persist. suppress/running_mean reset.")
    print()
    ...
    print()
    print("=" * 70)
    print(f"STEP 978 RESULTS (965 chain: LS20=67.0, 977 pending):")
