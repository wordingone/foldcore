"""
Step 704 — 674 with running-mean centering (adaptive), 10 seeds, 25s cap.

Step 698: running-mean centering rescued s4 (NO_L1 → 15544) vs frame-local (NO_L1).
Does running-mean centering help or hurt other seeds?
If ≥8/10: running-mean may be the better default centering mode.
...
class TT_RunningMean:
    """674 with running-mean centering (reset per episode) instead of frame-local centering."""
...
# Step 690: 674 frame-local centering per-seed L1 at 25s (for comparison)
STEP690_674 = {0: 18401, 1: 2847, 2: None, 3: 2402, 4: None,
...
def run(seed, make):
    ...
    bsl = STEP690_674.get(seed)
    if l1 and bsl:
        ratio = bsl / l1
        spd = f"{ratio:.1f}x faster" if ratio > 1 else f"{1/ratio:.1f}x slower"
    elif l1 and bsl is None:
        spd = "was_NO_L1_in_674"
    else:
        spd = "NO_L1"
    print(f"  s{seed}: L1={l1}({spd}) aliased={len(sub.aliased)} t={elapsed:.1f}s", flush=True)
...
def main():
    ...
    print(f"Step 704: 674 running-mean centering, {N_SEEDS} seeds (0-9), {CAP_SECONDS}s cap")
    print(f"Comparing: frame-local centering (674 Step 690) vs running-mean centering")
    ...
    for r in results:
        bsl = STEP690_674.get(r['seed'])
        bsl_str = f"674_frame_local={bsl}" if bsl else "674_frame_local=NO_L1"
        status = f"L1={r['l1']}" if r['l1'] else "NO_L1"
        print(f"  s{r['seed']}: running_mean={status} {bsl_str}")

    if l1_n >= 8:
        print("FINDING: Running-mean centering competitive or better — may be default centering mode")
    elif l1_n >= 6:
        print("MARGINAL: Running-mean helps some seeds but not enough to replace frame-local")
    else:
        print("KILL: Running-mean centering hurts overall performance")
