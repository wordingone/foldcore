"""
Step 698 — s4 centering diagnostic: 674 with running mean centering.

Standalone 674 uses frame-local centering (x - x.mean()).
Chain 674 uses running mean centering (x_raw - _mu, reset on episode end).

Step 693: s4 worked in chain (L1=2002) but not standalone (24968 or NO_L1 at 25s).
Hypothesis: running mean centering (with per-episode reset) is the variable.

Test: 674 with running-mean centering, seed 4 only, 25s cap.
Kill: L1 > 10000 (didn't fix the regression).
"""
import numpy as np
import time
import sys
...
class TT_RunningMean:
    """674 substrate with running-mean centering (as in chain) instead of frame-local centering."""
...
def run(seed, make):
    ...
    print(f"  s{seed}: {status} steps={step} aliased={len(sub.aliased)} t={elapsed:.1f}s", flush=True)
...
def main():
    ...
    print(f"Step 698: s4 centering diagnostic — 674 with running-mean centering, {CAP_SECONDS}s cap")
    print(f"Hypothesis: chain uses running-mean centering (reset per episode) = s4 fix")
    print(f"Standalone 674 uses frame-local centering (x - x.mean())")

    r = run(SEED, mk)

    print(f"\n{'='*60}")
    if r['l1'] and r['l1'] <= 10000:
        print(f"FINDING: Running-mean centering fixes s4. L1={r['l1']} (chain got 2002)")
        print(f"  Encoding context IS the variable. Frame-local vs running-mean centering = different hash.")
    elif r['l1']:
        print(f"PARTIAL: L1={r['l1']} — fixed but slow (>{10000}). Running mean helps but not fully.")
    else:
        print(f"KILL: Running-mean centering did not fix s4 (NO_L1 at {CAP_SECONDS}s)")
        print(f"  Centering is not the variable. Different mechanism at play.")
