"""
Step 713a — Universal Action Discovery on LS20.

674 + running-mean substrate. Combined action space: 4 directions + 64 grid clicks = 68 actions.
Action pruning: track state changes per action. After DEAD_THRESHOLD probes with 0 changes, mark DEAD.
Argmin over LIVE actions only.

Question: Does it discover that directional actions work and click actions are dead on LS20?
Kill criterion: L1 < 3/5 (scaled from 11/20) → action discovery actively harmful.
"""
import numpy as np
import time
import sys
...
MIN_VISITS_ALIAS = 3
WARMUP_STEPS = 500
DEAD_THRESHOLD = 5   # Mark dead after this many probes with 0 changes
MAX_STEPS = 120_001
N_SEEDS = 5
...
class UniversalAD:
    ...
    def act(self):
        ...
        if self.steps <= WARMUP_STEPS:
            # Round-robin over live actions
            idx = live_sorted[self._probe_ptr % len(live_sorted)]
            self._probe_ptr += 1
        else:
            # Argmin over live actions
...
def main():
    ...
    t_start = time.time()
    print(f"Step 713a: Universal action discovery on LS20, {N_SEEDS} seeds, {MAX_STEPS-1} steps")
    ...
    print(f"Kill criterion: L1 < 3/{N_SEEDS} (scaled from 11/20)")
    if l1_n >= 3:
        print(f"PASS: Action discovery viable on LS20 — run step 713b (FT09)")
    else:
        print(f"KILL: Action discovery harmful (L1={l1_n}/{N_SEEDS} < 3/{N_SEEDS})")
    print(f"Compare: running-mean 674 LS20 5 seeds expected ~5/5 (Step 705/708)")
