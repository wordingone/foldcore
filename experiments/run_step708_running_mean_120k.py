"""
Step 708 — 674 running-mean centering, 20 seeds, 120K steps.

Step 705: running-mean at 25s — coverage characterization.
Frame-local 674 at 120K = 20/20 (Step 699).
Does running-mean also reach 20/20 at 120K?
...
# Frame-local 674 at 120K (Step 699) for comparison
STEP699 = {0:18401,1:2847,2:20158,3:2402,4:24968,5:6272,6:20397,7:67359,8:126,9:11407,
           10:151,11:40442,12:50952,13:11880,14:9053,15:7565,16:64830,17:7456,18:746,19:11990}
...
def run(seed, make):
    ...
    bsl = STEP699.get(seed)
    if l1 and bsl:
        ratio = bsl / l1
        spd = f"{ratio:.1f}x faster" if ratio > 1 else f"{1/ratio:.1f}x slower"
    else:
        spd = "NO_L1" if not l1 else "no_baseline"
    print(f"  s{seed:2d}: L1={l1}({spd}) aliased={len(sub.aliased)} t={elapsed:.1f}s", flush=True)
...
def main():
    ...
    t_start = time.time()
    print(f"Step 708: 674 running-mean centering, {N_SEEDS} seeds, {MAX_STEPS-1} steps cap")
    print(f"Game: LS20 9607627b. Compare vs frame-local Step 699 (20/20).")
    results = []
    ...
    print(f"Compare: frame-local 674 at 120K = 20/20 (Step 699)")
    if l1_n >= 20: print("FINDING: Running-mean also achieves 20/20 at 120K")
    elif l1_n >= 18: print(f"FINDING: Running-mean reaches {l1_n}/20 at 120K (near complete)")
    else: print(f"KILL: Running-mean only {l1_n}/20 at 120K < 18/20 threshold")
