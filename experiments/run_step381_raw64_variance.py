#!/usr/bin/env python3
"""
Step 381 -- Raw 64x64 + per-dim variance weighting. LS20.

Weights = codebook per-dim variance. Timer=2 pixels high var + sprite=~50 med var.
Background=4000+ zero var suppressed. Sprite dominates weighted encoding.
2K steps. Kill: sim variance increase vs unweighted.
Script: scripts/run_step381_raw64_variance.py
"""
...
def main():
    t0=time.time()
    print(f"Step 381 -- Raw 64x64 + variance weighting on LS20. {D} dims.",flush=True)
    ...
    unique_states=set(); action_counts={}; sims_log=[]
    ...
    while ts<2000 and go<50:
        ...
        unique_states.add(hash(raw.numpy().tobytes()))
        ...
        if ts%500==0:
            ...
            n_active=(w>0.01).sum(); n_high=(w>0.5).sum()
            ...
            print(f"    [step {ts:5d}] cb={fold.V.shape[0]} thresh={fold.thresh:.6f}"
                  ...
                  f"  active_dims={n_active} high_dims={n_high}",flush=True)
            print(f"      actions: {action_counts}",flush=True)

    elapsed=time.time()-t0
    print(flush=True); print("="*60,flush=True)
    print("STEP 381 SUMMARY",flush=True); print("="*60,flush=True)
    ...
    if sims_log:
        ...
        print(f"\nWeighted sim: min={sl.min():.4f} max={sl.max():.4f} mean={sl.mean():.4f} std={sl.std():.4f}",flush=True)
        print(f"Compare 377 (unweighted): mean=0.984 std=0.009",flush=True)

    w=fold.weights.cpu().numpy()
    n_active=(w>0.01).sum(); n_high=(w>0.5).sum(); n_zero=(w<0.001).sum()
    print(f"\nWeight stats: active(>0.01)={n_active} high(>0.5)={n_high} zero(<0.001)={n_zero} / {D}",flush=True)

    # Show weight as 64x64 heatmap summary: which rows have high weights?
    w_grid=w.reshape(64,64)
    row_weights=w_grid.mean(axis=1)
    print(f"\nPer-row avg weight (top 10 rows):",flush=True)
    top_rows=np.argsort(row_weights)[-10:][::-1]
    for r in top_rows:
