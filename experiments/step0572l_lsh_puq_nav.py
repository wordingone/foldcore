"""
Step 572l — Puq navigation for L3.

572k: L2multi=5/5 confirmed (~960 L2/seed/min). Wall map frozen correctly.
Post-L2: agent in puq but no navigation → random walk → times out.

Puq puzzle (from ls20.py):
  hul=[12,9,14,8]  kdj=[0,90,180,270]
  pxr() init: snw=5(data["qqv"]=5), tmx=hul.index(12)=0, tuv=kdj.index(0)=0
  Win needs: snw==5✓, tmx==hul.index(9)=1, tuv==kdj.index(270)=3
  → 1 gic touch (tmx:0→1), 3 kdy touches (tuv:0→3), then lhs → L3!

Puq sprite tags:
  "gic" = qqv sprite at (29,45) → tmx += 1 (walk through it; stays in level)
  "bgt" = kdy sprite at (49,10) → tuv += 1
  "mae" = lhs sprite at (54,50) → win if state correct

Puq positions:
  SPAWN    = (9, 45)   pca spawn
  GIC_GRID = (29, 45)  qqv "gic" — touch ONCE then avoid
  KDY_GRID = (49, 10)  kdy "bgt" — touch 3×
  LHS_GRID = (54, 50)  lhs "mae" — win → L3 (entering tmx level)

State sequence:
  Phase 'gic': BFS to GIC_GRID. On arrival, add to avoid_set, phase='kdy'.
  Phase 'kdy': BFS to KDY_GRID. On kdj diff, puq_tuv++. After 3, add to avoid_set, phase='lhs'.
  Phase 'lhs': BFS to LHS_GRID. Arrival = L3!

Also fixes:
  - Spurious on_l2() after done=True: now checks prev_cl==1 before calling on_l2()
  - per_seed_cap=60s (5 seeds = 5 min)
"""
import numpy as np
import time
import sys
...
PUQ_KDY_GRID = (49, 10)   # "bgt" tag — touch 3× (tuv: 0→3)
...
def main():
    ...
    if l3n > 0:
        print(f"\nFIND: L3={l3n}/{len(R)}! puq navigation works!")
    elif l2n > 0:
        total_puq_hits = sum(r['puq_hits'] for r in R)
        total_puq_lhs = sum(r['puq_lhs'] for r in R)
        if total_puq_hits == 0:
            print(f"\nKILL: L3=0. puq_bfs+=0 — wall set not built or BFS fails. "
                  f"Check N_MAP_PUQ threshold (need {N_MAP_PUQ} L2s).")
