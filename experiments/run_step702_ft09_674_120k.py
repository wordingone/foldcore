"""
Step 702 — 674 on FT09, 20 seeds, 120K steps.

FT09: 5/5 L1 at 25s (Step 680), 5/5 at 300s (Step 691).
Aliasing frozen at 1-4 throughout 300s. L2=0/5.
Complete FT09 characterization: 20 seeds at 120K steps.
...
def run(seed, make):
    ...
    for step in range(1, MAX_STEPS):
        ...
        if cl > level:
            ...
            if cl == 2 and l2 is None:
                l2 = step
                print(f"  s{seed} L2@{l2} aliased={len(sub.aliased)}", flush=True)
    ...
    print(f"  s{seed:2d}: {status} L2={l2} aliased={len(sub.aliased)} t={elapsed:.1f}s", flush=True)
    return dict(seed=seed, l1=l1, l2=l2, aliased=len(sub.aliased))
...
def main():
    ...
    t_start = time.time()
    print(f"Step 702: 674 on FT09, {N_SEEDS} seeds, {MAX_STEPS-1} steps cap")
    print(f"Game: FT09 (check init log for version hash)")
    ...
    for r in results:
        ...
        print(f"  s{r['seed']:2d}: {status} L2={r['l2']} aliased={r['aliased']}")

    if l1_n >= 18: print("FINDING: 674 achieves near-complete L1 coverage on FT09 at 120K")
    elif l1_n >= 14: print("MARGINAL: Partial FT09 coverage at 120K")
    else: print(f"WEAK: L1={l1_n}/{N_SEEDS} on FT09")
