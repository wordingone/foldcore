"""
Step 707 — VC33 with running-mean 674, 5 seeds, 25s cap.

Step 681: frame-local 674 on VC33 = 0/5 (no aliasing for seeds 2-4).
Running-mean centering dramatically changes aliasing landscape (Step 704).
Does it help VC33?

Kill: still 0/5 — VC33 needs zone discovery, not aliasing resolution.
"""
import numpy as np
import time
import sys
...
# VC33: 8x8 click grid = 64 actions
...
def run(seed, make):
    env = make()
    sub = TT_RunningMean_VC33(seed=seed * 1000)
    ...
    print(f"  s{seed}: {status} aliased={len(sub.aliased)} t={elapsed:.1f}s", flush=True)
...
def main():
    ...
    print(f"Step 707: VC33 running-mean 674, 5 seeds, {CAP_SECONDS}s cap")
    print(f"Game: VC33 (check init log for version hash). Step 681 frame-local was 0/5.")
    results = []
    for seed in range(5):
    ...
    for r in results:
        ...
        print(f"  s{r['seed']}: {status} aliased={r['aliased']}")

    if l1_n >= 1:
        print("FINDING: Running-mean centering helps VC33 — aliasing change matters")
    else:
        print("KILL: Running-mean still 0/5 on VC33 — needs zone discovery, not aliasing")
