"""
Pure abstract model solver for LP85 - tracks ALL sprites at cycle positions.
"""
import sys
import json
import time
...
def solve():
    ...
    for level_idx in range(8):
        ...
        maps_data = izutyjcpih.get(level_name, {})

        # Find all cycle positions (sprite coords)
        all_cycle_positions = set()
        for map_name, grid in maps_data.items():
            for y, row in enumerate(grid):
                for x, val in enumerate(row):
                    if val != -1:
                        all_cycle_positions.add((x * crxpafuiwp, y * crxpafuiwp))

        # Find sprites at cycle positions
        moveable_sprites = []
        sprite_tags_list = []
        seen_sprites = set()
        for pos in sorted(all_cycle_positions):
            sprite = g.ttawusezqc(pos[0], pos[1])
            if sprite and id(sprite) not in seen_sprites:
                seen_sprites.add(id(sprite))
                moveable_sprites.append(sprite)
                tags = sprite.tags[0] if sprite.tags else ''
                sprite_tags_list.append(tags)

        print(f'  {len(moveable_sprites)} sprites, budget={step_budget}')

        # Find display coords for buttons
        btn_info = []
        ...
        print(f'  {len(btn_display)} buttons: {list(btn_display.keys())}')
        ...
        # Initial state = positions of all moveable sprites
        init_state = tuple((s.x, s.y) for s in moveable_sprites)

        # Win condition indices
        tile_b_idx = [i for i, t in enumerate(sprite_tags_list) if t == 'bghvgbtwcb']
        tile_f_idx = [i for i, t in enumerate(sprite_tags_list) if t == 'fdgmtkfrxl']
        goal_idx = [i for i, t in enumerate(sprite_tags_list) if t == 'goal']
        goal_o_idx = [i for i, t in enumerate(sprite_tags_list) if t == 'goal-o']

        print(f'  Tiles(b): {len(tile_b_idx)}, Tiles(f): {len(tile_f_idx)}, Goals: {len(goal_idx)}, Goals-o: {len(goal_o_idx)}')

        def check_win(state):
            goal_positions = set(state[i] for i in goal_idx)
            goal_o_positions = set(state[i] for i in goal_o_idx)
            for i in tile_b_idx:
                tx, ty = state[i]
                if (tx + 1, ty + 1) not in goal_positions:
                    return False
            for i in tile_f_idx:
                tx, ty = state[i]
                if (tx + 1, ty + 1) not in goal_o_positions:
                    return False
            return True
        ...
        while queue:
            ...
            if solved:
                break
            if explored > 20000000:
                print(f'  STATE LIMIT (e={explored})')
                break
