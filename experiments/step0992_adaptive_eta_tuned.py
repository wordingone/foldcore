"""
step0992_adaptive_eta_tuned.py -- Adaptive eta via h-novelty, tuned hyperparameters.

FAMILY: Adaptive learning rate (same as 991, retuned)
R3 HYPOTHESIS: Same as 991. 991 was maxed at 3x because EMA_H_EMA=0.1 too slow + SCALE=2.0 too high.
Fix: ETA_H_EMA=0.5 (faster EMA, more discriminative novelty) + SCALE=0.3 (gentler) + clip [0.8, 1.5].
Novel states: 1.5x ETA_W. Familiar states: 0.8x ETA_W. Truly adaptive rather than 3x-everywhere.
...
def run_chain(seeds, n_steps, cifar_steps, cifar_imgs, cifar_lbls):
    c1l, lsl, ftl, vcl, c2l = [], [], [], [], []
    for seed in seeds:
        sub = AdaptiveEtaTuned992(seed=seed)
...
if __name__ == "__main__":
    import os
    print("=" * 70)
    print("STEP 992 — ADAPTIVE ETA TUNED (EMA=0.5, SCALE=0.3, clip[0.8,1.5])")
    print("=" * 70)
    ...
    print(f"eta = ETA_W * clip(1 + {H_NOVELTY_SCALE}*||h-EMA_h||, {ETA_MIN_FACTOR}, {ETA_MAX_FACTOR})")
    print(f"ETA_H_EMA={ETA_H_EMA} (faster tracking). delta_per_action UNCHANGED.")
    ...
    print()
    print("=" * 70)
    print("STEP 992 RESULTS (965 chain baseline: LS20=67.0):")
