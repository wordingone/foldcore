"""
Step 1393 — Dendritic spatial STDP v2: feedback loop + dual-calcium.
Leo mail 3994 (corrections to 1392), 2026-03-30. Gate check: all 11 gates passed.

Two deviations from 1392 corrected:

DEVIATION 1 — Feedback loop in both try1 AND try2:
  1392: try1 used random actions; try2 used softmax(W).
  1393: BOTH tries use softmax(firing_rates). The self-organizing feedback IS the
  mechanism — random try1 actions produce noise that STDP can't filter. With the
  feedback loop, early random spikes → random clicks → some patches get LTP →
  those patches get clicked more → more LTP on truly responsive patches → convergence.
  Bootstrap with random when all firing rates ≈ 0.

DEVIATION 2 — Dual-calcium STDP:
  1392: single calcium trace accumulates from act_in events AND spike events.
  1393: dual trace:
    ca_act: += CA_PRE on act-driven spike (spike fired while act_in > 0)
    ca_obs: += CA_POST on obs-driven spike (spike fired while obs_delta > threshold)
  LTP only when ca_act > 0 AND ca_obs > THETA_LOW (conjunction required).
  Patches with obs change but no click → ca_obs rises but ca_act = 0 → no LTP.
  A spike can be both act- and obs-driven (conjunction = strongest signal).

Seeds: 14440-14469. 30 draws. (1392 used 14410-14439.)
Kill criteria (Leo mail 3994):
...
STEP        = 1393
N_DRAWS     = 30
DRAW_SEEDS  = [14440 + i for i in range(N_DRAWS)]
N_DIAG_INIT = 3
TRY1_STEPS  = 4000
TRY2_STEPS  = 4000
TRY2_SEED   = 4
...
# Dual-calcium STDP (dual-trace architecture, Leo mail 3994)
...
class DendriticSubunit:
    ...
    def step(self, obs_delta, act_in):
        ...
        # Dual-calcium STDP (Leo mail 3994):
        # ca_act: act-driven spike trace (pre-synaptic signal)
        # ca_obs: obs-driven spike trace (post-synaptic signal)
        ...
        if fired:
            ...
            if act_driven:
                self.ca_act += CA_PRE   # I spiked because of a click
            if obs_driven:
                self.ca_obs += CA_POST  # I spiked because of obs change
