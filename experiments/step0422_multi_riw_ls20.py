#!/usr/bin/env python3
"""
Step 422 — Hybrid (ReadIsWrite tau=0.01 + argmin) on LS20, 10K steps.

R3 hypothesis: Does sharper attention (tau=0.01 vs 0.1) improve navigation?
Base: same as 418f but tau=0.1→0.01. Ran March 18 2026 as inline exec.

Result: unique=3335, dom=27%, cb=4503, spawn=45%. 14s.
KILL criteria passed (unique>3000, dom<40%), extend not met (unique<3500).
tau=0.01 gives ~same result as tau=0.1 (418f: unique=3248, dom=28%).
FINDING: tau doesn't matter for navigation. Action mechanism (argmin) is load-bearing.

Recovery: reconstructed from Bash exec command in ee4c4227:L3336.
"""
...
def main():
    ...
    print(f"Step 422: RIW Hybrid (distributed update + argmin + error spawn)")
    ...
    total = sum(action_counts)
    dom = max(action_counts) / total * 100 if total else 0
    spawn_rate = sub.n_spawns / ts * 100 if ts else 0

    print(f"\n{'='*60}")
    print("STEP 422 RESULTS")
    print(f"{'='*60}")
    ...
    kills = []
    if len(unique) < 3000: kills.append(f"unique={len(unique)}<3000")
    if dom > 40: kills.append(f"dom={dom:.0f}%>40%")
    if kills:
        print(f"KILL: {', '.join(kills)}")
    else:
        print("PASS: all kill criteria clear")
        if len(unique) > 3500 and dom < 30:
            print("EXTEND: unique>3500 AND dom<30% -> run to 30K")
