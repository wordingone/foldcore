"""
Step 713b (raw) — Universal Action Discovery on FT09, raw 64x64.

R3 hypothesis: action pruning is self-derived action-space discovery (I3).
The substrate discovers which actions produce state changes from its own interaction,
modifying its effective action space. For FT09: directions should be DEAD (no state change),
grid clicks should be LIVE (produce visual state changes on the 64x64 canvas).

Same architecture as 713_raw: 674+running-mean, k=12+k=20, raw 64x64 (4096D), 68 universal actions.
Warmup 500 steps round-robin. Dead threshold: 5 probes with 0 changes.

Kill: 0/5 bootloader pass AND live_click=0 → click discovery failed.
"""
import numpy as np
import time
import sys
...
def run(seed, make):
    ...
    print(f"  s{seed:2d}: {bootloader} live={len(sub.live_actions)}(dir={live_dir},click={live_click}) dead={len(sub.dead_actions)} aliased={len(sub.aliased)} t={elapsed:.1f}s", flush=True)
...
def main():
    ...
    t_start = time.time()
    print(f"Step 713b (raw): Universal AD on FT09, {N_SEEDS} seeds, {MAX_STEPS-1} steps")
    print(f"Encoding: raw 64x64 = {DIM}D. R3: action pruning = self-derived action-space discovery.")
    print(f"Expected: dir actions DEAD, grid clicks LIVE.")
    results = []
    ...
    # R3 result: did it discover the right action space?
    correct_discovery = sum(1 for r in results if r['live_dir'] == 0 and r['live_click'] > 0)
    print(f"R3 result: {correct_discovery}/{N_SEEDS} seeds correctly discovered click-only action space")
    if correct_discovery >= 3:
        print(f"PASS: Action discovery identifies FT09 action space — proceed to VC33 (step 714)")
    else:
        dirs_live = sum(r['live_dir'] > 0 for r in results)
        print(f"NOTE: {dirs_live}/{N_SEEDS} seeds kept dir actions live — discovery incomplete")
