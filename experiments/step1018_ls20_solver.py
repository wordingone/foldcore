"""
step1018_ls20_solver.py — LS20/9607627b Multi-Level BFS Solver (Direction 2, Sub-mode A).

SUBSTRATE-ONLY FILE. No main(), no harness, no run loop.
Run via: python run_experiment.py --step 1018 --substrate experiments/step1018_ls20_solver.py --steps 10000 --seeds 5
...
DIRECTION 2: Per-game prescription allowed (Jun directive 2026-03-24).
  ...
  - Hardcoded BFS solutions replayed for each level

PURPOSE: LS20 multi-level ceiling measurement. How many levels can a source-aware BFS solver clear?
  Expected: 7/7 levels if BFS path fits within step budget + collectibles.

GAME ANALYSIS (Leo, mail 2994):
  - 7 levels (win_levels=7)
  - 64×64 grid, 4 directional actions, player 5×5 pixels
  - StepCounter=42, StepsDecrement=1 or 2 → 21-42 moves per life
  - 3 lives per level attempt
  - Win: navigate to all goal sprites (rjlbuycveu) with correct shape/color/rotation
  - Level 7: Fog=True (partial observability, doesn't affect planning)

KILL: N/A — diagnostic only.
EXPECTED: 7/7 levels L1 if BFS paths fit within budgets.
BUDGET: 10K steps, 5 seeds.
"""
import sys
import os
import types
...
print("Step 1018: Computing BFS solutions for LS20/9607627b levels...")
...
class Ls20SolverSubstrate:
    ...
    def on_level_transition(self):
        """Called when level is completed — advance to next level's solution."""
        self._level_idx += 1
        self._step_in_seq = 0
