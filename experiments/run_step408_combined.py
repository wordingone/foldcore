#!/usr/bin/env python3
"""
Step 408 -- Mask + centered unnorm + count-based selection. ALL pieces combined.

1. Temporal mask (Steps 400-401): ~100 informative dims
2. Centered unnormalized (Step 388): position discrimination
3. Count-based action selection (Step 407): per-winner action cycling

LS20. 64x64. 50K steps.
Script: scripts/run_step408_combined.py
"""
...
class CombinedFold:
    ...
    def process(self, x):
        """Combined: mask + centered unnorm + count-based selection."""
        x=x.to(self.dev).float()
        ...
        # Spawn/attract (standard class vote for labeling)
...
def main():
    t0=time.time()
    print(f"Step 408 -- Combined: mask + centered unnorm + count-based. 64x64. 50K. LS20.", flush=True)
    ...
    print("STEP 408 SUMMARY", flush=True); print("="*60, flush=True)
