"""
Step 572k — Post-L2 fix: freeze wall map, handle puq navigation.

572j: L2=5/5 confirmed! But post-L2 stall: cyc freezes after first L2.

Root cause:
  No on_l2() handler. When cl goes 1→2 (mgu→puq):
    - game_level stays 1
    - puq frames accumulate into l2_freq (the mgu wall map!)
    - Wall map corrupted → BFS fails → dr stays at spawn → no more L1/L2
  After game resets, on_l1() rebuilds wall_set from corrupted l2_mode → broken.

Fix:
  1. on_l2(): set game_level=2, set _wall_frozen=True
  2. _update_bg(): game_level==2 → l3_freq/l3_mode (puq mode tracking)
  3. on_l1(): if _wall_frozen, skip wall_set rebuild (use frozen wall_set)
  4. act(): game_level==2 → cluster navigation from l3_mode (like krg)
  5. Main loop: detect cl==2 transition → on_l2()

Goal: see multiple L2 completions per run, verify wall map survives across games.
"""
import numpy as np
import time
import sys
...
class SubDual:
    def __init__(self, seed=0):
        ...
        self.l2_frames = 0
        self.l3_freq = np.zeros((64, 64, 16), dtype=np.int32)  # puq mode
        self.l3_mode = np.zeros((64, 64), dtype=np.int32)
        self.l3_frames = 0
        self.game_level = 0
        self.l1_cycles = 0
        self.l2_count = 0   # number of L2 completions
        self.l1_targets = []; self.l3_targets = []
        self.visited = []; self.agent_yx = None; self.prev_arr = None
        self._steps_since_detect = 99999
        self.target_actions = 0
        self.n_l1_tgt = 0
        # State sequencing (mgu)
        self._tuv_est = 0
        self._phase = 'kdy'
        self._kdy_touches = 0
        self._lhs_approaches = 0
        # Dead reckoning
        self._dr_pos = None
        self._wall_set = None
        self._wall_frozen = False   # True after first L2 — don't corrupt wall map
        self._bfs_path = []
        # Diagnostics
    ...
    def observe(self, frame):
        ...
        if self.game_level == 0:
            frames = self.l1_frames
        elif self.game_level == 1:
            frames = self.l2_frames
        else:
            frames = self.l3_frames
        if frames < WARMUP:
    ...
    def on_l1(self):
        ...
        self._bfs_path = []
        if self._wall_frozen:
            # Wall set already frozen from first L2 run — don't rebuild
            pass
        elif self.l1_cycles >= N_MAP and self.l2_frames >= WARMUP:
            self._wall_set = build_wall_set(self.l2_mode)

    def on_l2(self):
        """Called when cl transitions from 1→2 (mgu→puq)."""
        self.game_level = 2
        self.l2_count += 1
        self.visited = []
        self.agent_yx = None
        self._steps_since_detect = 99999
        # Freeze the wall set — don't let puq frames corrupt it
        if not self._wall_frozen and self._wall_set is not None:
            self._wall_frozen = True
    ...
    def on_reset(self):
        ...
        self._tuv_est = 0; self._phase = 'kdy'
        self._bfs_path = []
        # _wall_set and _wall_frozen persist across resets
    ...
    def act(self):
        ...
        if self.game_level == 2 and (self._steps_since_detect >= 500
                and self.l3_frames >= WARMUP):
            self.l3_targets = find_isolated_clusters(self.l3_mode)
            self._steps_since_detect = 0
        ...
        elif self.game_level in (0, 2):
            targets = self.l1_targets if self.game_level == 0 else self.l3_targets
...
def t0():
    ...
    assert len(path) == 18, f"lhs path: {len(path)}"
    path2 = bfs_path(SPAWN, KDY_GRID, walls)
    assert len(path2) == 18, f"kdy path: {len(path2)}"
    print("T0 PASS")
...
def main():
    ...
    for seed in range(n_seeds):
        ...
        l1_step = l2_steps = []; go = 0; seed_start = time.time()
        prev_cl = 0
        ...
        for step in range(1, 200_001):
            ...
            if cl > prev_cl:
                ...
                if cl == 2:
                    ...
                    print(f"  s{seed} L2@{step}! count={sub.l2_count} "
                          f"tuv_est={sub._tuv_est} frozen={sub._wall_frozen} go={go}", flush=True)
            prev_cl = cl

            if step % 10_000 == 0:
                el = time.time() - seed_start
                print(f"  s{seed} @{step} cyc={sub.l1_cycles} l2c={sub.l2_count} "
                      f"l2f={sub.l2_frames} l3f={sub.l3_frames} "
                      f"phase={sub._phase} frozen={sub._wall_frozen} "
                      f"bfs+={sub.bfs_hits} bfs-={sub.bfs_fails} go={go} {el:.0f}s", flush=True)
            if time.time() - seed_start > per_seed_cap:
                print(f"  s{seed} cap @{step}", flush=True); break

        R.append(dict(seed=seed, l1=l1_step, l2_count=sub.l2_count, go=go, steps=step,
                      cycles=sub.l1_cycles, l2_frames=sub.l2_frames, l3_frames=sub.l3_frames,
                      bfs_hits=sub.bfs_hits, bfs_fails=sub.bfs_fails,
                      frozen=sub._wall_frozen, ta=sub.target_actions))

    elapsed = time.time() - t_start
    print(f"\n{'='*60}\nElapsed: {elapsed:.0f}s")
    for r in R:
        tag = f"L2x{r['l2_count']}" if r['l2_count'] > 0 else ("L1" if r['l1'] else "---")
        print(f"  s{r['seed']}: {tag}  cyc={r['cycles']}  l2c={r['l2_count']}  "
              f"l2f={r['l2_frames']}  l3f={r['l3_frames']}  "
              f"bfs+={r['bfs_hits']}  bfs-={r['bfs_fails']}  "
              f"frozen={r['frozen']}  go={r['go']}  ta={r['ta']}")
    l1n = sum(1 for r in R if r['l1'])
    l2n = sum(1 for r in R if r['l2_count'] > 0)
    l2_multi = sum(1 for r in R if r['l2_count'] > 1)
    print(f"\nL1={l1n}/{len(R)}  L2={l2n}/{len(R)}  L2multi={l2_multi}/{len(R)}")
    if l2_multi > 0:
        print(f"\nFIND: Multiple L2 completions! Wall map survives across games.")
    elif l2n > 0:
        print(f"\nFIND: L2 fires but only once — check post-L2 stall still present.")
    else:
        print(f"\nKILL: L2=0. Regression — check on_l2() handler and wall_frozen logic.")
