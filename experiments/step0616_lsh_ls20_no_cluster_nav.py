"""
Step 616: LS20 fix — remove L0 cluster targeting + random tie-breaking + no G reset.

Root cause found in probe8: agent_yx frozen at (57.39, 5.39) for 2600+ steps.
  - L0 cluster targeting uses agent_yx to navigate to clusters via dir_action
  - agent_yx tracks animation artifacts (game visual noise at fixed position), not agent
  - Cluster nav always targets same direction from frozen ayx → hits wall → no movement
  - done never fires after first 2 episodes

Fix: Remove L0 cluster targeting entirely from act().
  Pure argmin + random tie-breaking for L0 exploration.
  Probe6 (no cluster nav) confirmed: done=7 in 1000 steps (every ~143 steps).
  Expected: 800+ episodes in 5 min → L0 fully explored → L1 reachable.

Also keeps:
  - Random tie-breaking in argmin (615 fix)
  - No G reset on level transitions (614 fix)
  - L3/L4 bootstrap (Option C) from 612/613

2 seeds × 5 min. Signal: L1 >= 1/2 (primary), L2 >= 1/2 (stretch). Kill: L1 = 0/2.
"""
import numpy as np
import time
import sys
...
class SubDual:
    ...
    def on_l1(self):
        # FIX: do NOT reset self.G — let graph accumulate across level transitions (572u behavior)
        self.game_level = 1; self.l1_cycles += 1
        self.visited = []; self._pn = None
    ...
    def on_l4(self):
        # FIX: do NOT reset self.G
        self.l4_count += 1; self.visited = []; self._pn = None
        self.agent_yx = None
    ...
    def act(self):
        # L0 cluster targeting REMOVED (step 616 fix):
        # agent_yx tracks animation artifacts (frozen at wrong position → cluster nav hits wall)
        # Pure argmin + random tie-breaking for L0 exploration instead
        ...
        if self.game_level == 1:
            ...
            if action is not None:
                ...
                self._pn = self._cn; self._pa = action
                self.target_actions += 1; return action
        # L0 cluster nav removed — fall through to argmin
...
def main():
    ...
    t_start = time.time()
    print("Step 616: LS20 no-L0-cluster-nav + random tie-break + no-G-reset", flush=True)
    print(f"  Fix: removed L0 cluster targeting (ayx frozen at animation artifact)", flush=True)
    print(f"  Also: random tie-break, no G reset, L3/L4 bootstrap", flush=True)
    ...
    for seed in range(n_seeds):
        ...
        R.append({'seed': seed, 'l1c': sub.l1_cycles, 'l2c': sub.l2_count,
                  'l3c': sub.l3_count, 'l4c': sub.l4_count,
                  'l1_step': l1_step[0], 'l3_steps': l3_steps, 'l4_steps': l4_steps,
    ...
    print(f"\n{'='*60}", flush=True)
    print(f"Step 616: LS20 no-cluster-nav", flush=True)
    l2_wins = sum(1 for r in R if r['l2c'] > 0)
    l3_wins = sum(1 for r in R if r['l3c'] > 0)
    l4_wins = sum(1 for r in R if r['l4c'] > 0)
    print(f"  L2: {l2_wins}/{len(R)}  L3: {l3_wins}/{len(R)}  L4: {l4_wins}/{len(R)}", flush=True)
    for r in R:
    ...
    if l4_wins >= 1:
        print("  SIGNAL: L4 reached! Full bootstrap chain works.", flush=True)
    elif l3_wins >= 1:
        print("  L3 reached. L4 bootstrap needs more cycles.", flush=True)
    elif l2_wins >= 1:
        print("  L2 reached. No-cluster-nav fix confirmed.", flush=True)
    else:
        l1_wins = sum(1 for r in R if r.get('l1c', 0) > 0)
        print(f"  L2 not reached. Check if L1 was hit.", flush=True)
