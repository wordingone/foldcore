"""
sub1094_prosecution_v24.py — Forward model with extended warmup

SUBSTRATE-ONLY FILE. No main(), no harness, no run loop.
Run via: python run_experiment.py --step 1094 --substrate experiments/sub1094_prosecution_v24.py

FAMILY: Forward-model action selection (same as v23)
Tagged: prosecution (ℓ_π)
R3 HYPOTHESIS: Extended warmup (100 steps vs v23's 10) gives W_fwd enough
training data to predict per-action effects reliably. v23 got 3/10 seeds —
hypothesis: short warmup means some seeds miss informative actions in first
10 random steps. 100 steps → every action sampled ~14 times.

ONE CHANGE FROM v23: warmup 10 → 100 steps.
Everything else IDENTICAL.

KILL: seed success rate <= 3/10 (no improvement from warmup).
SUCCESS: seed success rate >= 5/10 OR ARC > 0.01/game.
"""
import sys
...
class ForwardModelSubstrate:
    ...
    def _select_action(self, enc_w):
        ...
        if self.step_count < 100:  # extended warmup (v23 used 10)
...
CONFIG = {
    ...
    "tag": "prosecution v24 (ℓ_π forward model + 100-step warmup)",
}
