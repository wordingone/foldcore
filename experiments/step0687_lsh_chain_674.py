"""
Step 687 — Chain benchmark with 674 substrate (transition-triggered dual-hash).

Chain: CIFAR-100 P1 (1K) -> LS20 (L1) -> CIFAR-100 P3 (1K).
Per-domain centering (on_reset) as in Step 546.
k=12 nav + k=20 fine at aliased cells.

Report: CIFAR P1/P3 accuracy, LS20 L1 step, aliased_cells in each phase.
Prediction: CIFAR = 0 aliased cells (deterministic transitions).
...
def cifar_phase(sub, images, labels, desc):
    ...
    print(f"  {desc}: nodes={len(node_label)} cells={len(sub.live)} "
          f"aliased={aliased_before}->{aliased_after}", flush=True)
    return node_label
...
def run_chain(seed, make_ls20, cifar_images, cifar_labels):
    ...
    for step in range(1, LS20_STEPS + 1):
        ...
        if time.time() - t_ls > TIME_CAP: break
...
def main():
    ...
    print(f"Step 687: Chain with 674 substrate. {CIFAR_N} CIFAR obs per phase, 300s LS20 cap.")
    R = []
    for seed in range(3):
    ...
    print(f"L1={l1n}/3  avg_CIFAR_acc={avg_acc:.1%}")
    for r in R:
        print(f"  s{r['seed']}: L1={r['l1']} cifar1_aliased={r['cifar1_aliased']} "
              f"ls20_aliased={r['ls20_aliased']} acc={r['acc']:.1%}")
    if all(r['cifar1_aliased'] == 0 for r in R):
        print("CONFIRMED: Zero aliased cells during CIFAR — fine hash stays dormant")
    else:
        print(f"WARNING: CIFAR phase has aliased cells — domain contamination possible")
    if l1n >= 2:
        print("FINDING: 674 mechanism compatible with chain benchmark")
    else:
        print("KILL: Chain disrupts 674 mechanism")
