"""Step 582c: 582 with op2 DISABLED. Only op0+op3 (surprise boost). If 4/5 -> op3 alone drives gain. If 3/5 -> op2 alone drives it."""
import time, numpy as np, sys
K=12; DIM=256; N_A=4; MAX_STEPS=50_000; TIME_CAP=280; BONUS=50
...
class NoOp2Sub:
    ...
    def observe(self,frame):
        ...
        if self._prev_node is not None:
            ...
            elif pred==node: self.edge_op[key]=0
            else: self.edge_pred[key]=node; self.edge_op[key]=3  # surprise boost, NO death penalty
    def on_death(self): self.total_deaths+=1  # no op2
    def act(self):
        node=self._curr_node
        eff=[max(0,sum(self.G.get((node,a),{}).values())-BONUS) if self.edge_op.get((node,a),0)==3
             else sum(self.G.get((node,a),{}).values()) for a in range(N_A)]
...
def main():
    ...
    print("Step 582c: 582 with op2 disabled (only op0+op3)",flush=True)
    r1=[]; t=time.time()
    for seed in range(5):
        if time.time()-t>1380: break
        print(f"\nseed {seed}:",flush=True); r1.append(run_seed(mk,seed,NoOp2Sub))
    r2=[]
    for seed in range(5):
        if time.time()-t>1380: break
        print(f"\nseed {seed}:",flush=True); r2.append(run_seed(mk,seed,ArgminSub))
    l1a=sum(r['l1'] for r in r1); l1b=sum(r['l1'] for r in r2)
    print(f"\n{'='*60}\nStep 582c: op2 disabled")
    print(f"  NoOp2:  {sum(1 for r in r1 if r['l1']>0)}/5 seeds L1, total={l1a}")
    ...
    if l1a>l1b: print(f"\nSIGNAL: no-op2 ({l1a}) > argmin ({l1b})")
    elif l1a==l1b: print(f"\nNEUTRAL: no-op2 ({l1a}) == argmin ({l1b})")
    else: print(f"\nFAIL: no-op2 ({l1a}) < argmin ({l1b})")
