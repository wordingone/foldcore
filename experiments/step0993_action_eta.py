"""
step0993_action_eta.py -- Per-action adaptive eta via h-novelty accumulation.

FAMILY: Adaptive learning rate (992 base + action-dependent modulation)
R3 HYPOTHESIS: action_eta_boost[a] accumulates h-novelty when action a was taken.
Actions that consistently lead to novel states get higher W_pred learning rate.
Correct FT09 clicks → novel states → high eta_boost for those actions → W_pred adapts
faster → prediction error drops → alpha concentrates on changed dims → 800b signal.

Ban-safe: action_eta_boost is per-action GLOBAL (same structure as running_mean[a]).
Not per-state data. Doesn't touch delta_per_action.

Kill: LS20 < 65.9 (992 baseline). Success: FT09 > 0 OR VC33 > 0.
...
ETA_H_EMA = 0.50; H_NOVELTY_SCALE = 0.3
ETA_MIN_FACTOR = 0.8; ETA_MAX_FACTOR = 1.5
ACTION_ETA_DECAY = 0.95; ACTION_ETA_UPDATE = 0.05
...
class ActionEta993:
    def __init__(self, seed):
        ...
        self.action_eta_boost = np.ones(4, dtype=np.float32)

    def set_game(self, n_actions):
        self.h = np.zeros(H_DIM, dtype=np.float32)
        self._n_actions = n_actions
        self.delta_per_action = np.full(n_actions, INIT_DELTA, dtype=np.float32)
        self.action_eta_boost = np.ones(n_actions, dtype=np.float32)
        self._prev_ext = None; self._prev_action = None
    ...
    def process(self, obs):
        ...
        if self._prev_ext is not None and self._prev_action is not None:
            ...
            a = self._prev_action
            # Update per-action eta boost
            self.action_eta_boost[a] = (ACTION_ETA_DECAY * self.action_eta_boost[a]
                                        + ACTION_ETA_UPDATE * (1.0 + H_NOVELTY_SCALE * h_novelty))
            eta_adaptive = ETA_W * float(np.clip(self.action_eta_boost[a],
...
if __name__ == "__main__":
    import os
    print("=" * 70)
    print("STEP 993 — PER-ACTION ADAPTIVE ETA VIA H-NOVELTY")
    print("=" * 70)
    ...
    print(f"action_eta_boost[a] accumulates h-novelty per action. eta=ETA_W*clip(boost,{ETA_MIN_FACTOR},{ETA_MAX_FACTOR})")
    print(f"delta_per_action UNCHANGED. Kill: LS20<65.9  Success: FT09>0 OR VC33>0")
    print()
    ...
    print()
    print("=" * 70)
    print("STEP 993 RESULTS (992 baseline: LS20=65.9):")
