"""
step0991_adaptive_eta.py -- Adaptive W_pred learning rate via h-novelty.

FAMILY: Adaptive learning rate (orthogonal to delta_per_action)
R3 HYPOTHESIS: W_pred's learning rate modulated by how novel the current recurrent state is.
Novel h (far from recent EMA_h) → W_pred learns FAST. Familiar h → W_pred learns SLOW.
eta_adaptive = ETA_W * clip(1 + SCALE * ||h - EMA_h||, 0.5, 3.0)

Orthogonality argument:
- delta_per_action = state_change = ||(ext_enc - prev_ext) * alpha|| — NOT directly touched
- Indirect path: adaptive eta → different pred_errors → different alpha → different delta (2 steps)
- Fundamentally different from all 25 kills (which directly augmented delta_per_action)

h-novelty measure: ||h - EMA_h|| where EMA_h = 0.9*EMA_h + 0.1*h_current (global running mean)
Not per-state data — global running statistic. Ban-safe.

Doesn't fix temporal credit assignment for FT09. But first genuinely orthogonal mechanism.
If LS20 survives: confirms new degree of freedom exists.

SCALE=2.0 (if ||h - EMA_h||=1.0, eta doubles). Clipped to [0.5, 3.0]×ETA_W.
...
ETA_H_EMA = 0.10; H_NOVELTY_SCALE = 2.0
ETA_MIN_FACTOR = 0.5; ETA_MAX_FACTOR = 3.0
...
class AdaptiveEta991:
    """965 exact base + adaptive W_pred learning rate via h-novelty.
    eta_adaptive = ETA_W * clip(1 + SCALE * ||h - EMA_h||, 0.5, 3.0).
    delta_per_action and action selection unchanged."""
    ...
    def __init__(self, seed):
        ...
        self._h_ema = np.zeros(H_DIM, dtype=np.float32)  # running mean of h
    ...
    def set_game(self, n_actions):
        ...
        self._prev_ext = None; self._prev_action = None
        # h_ema persists across games (global running statistic)
    ...
    def _encode(self, obs):
        ...
        # Update h EMA after computing new h
    ...
    def process(self, obs):
        ...
        if self._prev_ext is not None and self._prev_action is not None:
            # Compute h-novelty: ||h - EMA_h||
            h_novelty = float(np.linalg.norm(self.h - self._h_ema))
            eta_factor = float(np.clip(1.0 + H_NOVELTY_SCALE * h_novelty,
                                       ETA_MIN_FACTOR, ETA_MAX_FACTOR))
            eta_adaptive = ETA_W * eta_factor

            # Standard 965 prediction + error
    ...
    def h_novelty_mean(self):
        return float(np.linalg.norm(self.h - self._h_ema))
...
def run_chain(seeds, n_steps, cifar_steps, cifar_imgs, cifar_lbls):
    ...
    for seed in seeds:
        ...
        print(f"  seed={seed}: C1={c1:.3f} LS20={l:4d} FT09={f:4d} VC33={v:4d} C2={c2:.3f}"
              f"  alpha_conc={sub.alpha_conc():.1f}  h_nov={sub.h_novelty_mean():.3f}")
...
if __name__ == "__main__":
    import os
    print("=" * 70)
    print("STEP 991 — ADAPTIVE ETA VIA H-NOVELTY")
    print("=" * 70)
    ...
    print(f"eta_adaptive = ETA_W * clip(1 + {H_NOVELTY_SCALE}*||h-EMA_h||, {ETA_MIN_FACTOR}, {ETA_MAX_FACTOR})")
    print(f"EMA_h decay={ETA_H_EMA}. delta_per_action UNCHANGED from 965.")
    ...
    print()
    print("=" * 70)
    print("STEP 991 RESULTS (965 chain baseline: LS20=67.0):")
