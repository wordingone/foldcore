"""
Step 614: LS20 L2 fix — remove G reset on level transitions.

Root cause found in 612b: go=2 in 111K steps (agent stuck in L1 for ~55K steps each).
  - 612b (606-based) resets self.G = {} in on_l1/on_l2/on_l3/on_l4
  - 572u does NOT reset G on level transitions — G accumulates throughout run
  - Result: argmin navigation in L1 starts from scratch each entry → agent can't
    complete L1 → l1_cycles never reaches N_MAP=30 → mgu never bootstraps → deadlock

Fix: Remove self.G = {} from on_l1, on_l2, on_l3, on_l4.
  G accumulates continuously (as in 572u), giving argmin better navigation each episode.
  Restores the 572u behavior that successfully reached L2/L3.

Includes all previous fixes:
  - L3 bootstrap: puq wall_set built from l3_mode mid-L3 run (612 fix)
  - L4 bootstrap: cluster targets built from l4_mode mid-L4 run (613 fix)
...
def main():
    ...
    t_start = time.time()
    print("Step 614: LS20 no-G-reset fix (restores 572u behavior)", flush=True)
    print(f"  Fix: G accumulates across level transitions (was reset on each on_l1/l2/l3/l4)", flush=True)
    print(f"  Also: L3/L4 bootstrap (Option C) from 612/613", flush=True)
    ...
    print(f"\n{'='*60}", flush=True)
    print(f"Step 614: LS20 no-G-reset", flush=True)
    ...
    if l4_wins >= 1:
        ...
        print("  L2 reached. G-reset fix confirmed. L3 bootstrap still bootstrapping.", flush=True)
    else:
        print("  FAIL: L2 not reached. Check L1 navigation with G accumulation.", flush=True)
