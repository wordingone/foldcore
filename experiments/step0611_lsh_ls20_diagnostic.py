"""
Step 611: LS20 L2->L3 state diagnostic.

Dump mgu pipeline state at both transitions:
  L1->L2 (which works: 572k L2=5/5)
  L2->L3 (which fails in 606)

Dumps at each transition:
  a) hash cells active (live set size + sample)
  b) edge count distribution (G size, avg/max per node-action)
  c) mode map summary (unique colors, nonzero region)
  d) cluster list (first 5 clusters)
  e) state variables that reset / wall_set status

Hypothesis: l3_frames=0 at first L2->L3 means _puq_wall_set never gets built.
5 seeds x 60s cap.
"""
import numpy as np
import time
...
def dump_state(sub, label, step, seed):
    """Dump pipeline state for diagnostics."""
    print(f"\n  ===== DUMP [{label}] seed={seed} step={step} =====", flush=True)

    # (a) Hash cells active
    live_n = len(sub.live)
    sample = list(sub.live)[:5]
    print(f"  a) hash_cells_active: {live_n}  sample={[hex(x)[:12] if isinstance(x,int) else str(x)[:20] for x in sample]}", flush=True)

    # (b) Edge count distribution
    g_node_actions = len(sub.G)
    if sub.G:
        counts = [sum(d.values()) for d in sub.G.values()]
        avg_c = np.mean(counts); max_c = max(counts)
        # Edge density: unique (node,action) pairs with nonzero outgoing edges
        nonzero = sum(1 for d in sub.G.values() if sum(d.values()) > 0)
        print(f"  b) graph: {g_node_actions} (node,action) pairs, avg_count={avg_c:.1f}, max={max_c}, nonzero={nonzero}", flush=True)
    else:
        print(f"  b) graph: EMPTY (just reset)", flush=True)

    # (c) Mode map summary by level
    levels = [
        ('l1', sub.l1_mode, sub.l1_frames),
        ('l2', sub.l2_mode, sub.l2_frames),
        ('l3', sub.l3_mode, sub.l3_frames),
        ('l4', sub.l4_mode, sub.l4_frames),
    ]
    for lname, mode, frames in levels:
        if frames == 0:
            print(f"  c) {lname}_mode: frames=0 (never seen)", flush=True)
            continue
        colors_present = np.unique(mode)
        nonzero_cells = int((mode > 0).sum())
        # Build clusters to compare layouts
        clusters = find_isolated_clusters(mode) if frames >= WARMUP else []
        print(f"  c) {lname}_mode: frames={frames}, colors={list(colors_present)}, nonzero_cells={nonzero_cells}, clusters={len(clusters)}", flush=True)

    # (d) Cluster list for current active mode
    if sub.game_level == 0: cur_mode, cur_frames = sub.l1_mode, sub.l1_frames
    elif sub.game_level == 1: cur_mode, cur_frames = sub.l2_mode, sub.l2_frames
    elif sub.game_level == 2: cur_mode, cur_frames = sub.l3_mode, sub.l3_frames
    else: cur_mode, cur_frames = sub.l4_mode, sub.l4_frames

    if cur_frames >= WARMUP:
        clusters = find_isolated_clusters(cur_mode)
        print(f"  d) clusters (current level mode, {len(clusters)} total):", flush=True)
        for c in clusters[:5]:
            print(f"       cy={c['cy']:.1f} cx={c['cx']:.1f} color={c['color']} size={c['size']}", flush=True)
        if len(clusters) > 5:
            print(f"       ... +{len(clusters)-5} more", flush=True)
    else:
        print(f"  d) clusters: mode not ready (frames={cur_frames} < {WARMUP})", flush=True)

    # (e) State variables & wall set status
    print(f"  e) game_level={sub.game_level}", flush=True)
    print(f"     mgu_wall_set={'built' if sub._mgu_wall_set is not None else 'None'} ({len(sub._mgu_wall_set) if sub._mgu_wall_set else 0} walls) frozen={sub._mgu_wall_frozen}", flush=True)
    print(f"     puq_wall_set={'built' if sub._puq_wall_set is not None else 'None'} ({len(sub._puq_wall_set) if sub._puq_wall_set else 0} walls) frozen={sub._puq_wall_frozen}", flush=True)
    print(f"     l1_cycles={sub.l1_cycles} l2_count={sub.l2_count} l3_count={sub.l3_count}", flush=True)
    print(f"     mgu_phase={sub._mgu_phase} mgu_dr_pos={sub._mgu_dr_pos}", flush=True)
    print(f"     puq_wp_idx={sub._puq_wp_idx} puq_dr_pos={sub._puq_dr_pos}", flush=True)
    print(f"     l1_targets={len(sub.l1_targets)} l4_targets={len(sub.l4_targets)}", flush=True)

    # Key diagnostic: will puq nav work at L3?
    if sub.game_level == 2:
        can_nav = sub._puq_wall_set is not None
        print(f"  ** PUQ NAV VIABLE: {can_nav}  (wall_set={'built' if can_nav else 'None, needs l3_frames>=' + str(WARMUP) + ' currently=' + str(sub.l3_frames)})", flush=True)
...
class SubDual:
    def __init__(self, seed=0):
        ...
        self._puq_lhs_ap = 0
        # Diagnostic: track transition steps
        self._l1_transition_step = None
        self._l2_transition_step = None
    ...
    def on_l1(self, step, seed):
        ...
        self._l1_transition_step = step
        dump_state(self, 'L1->L2', step, seed)

    def on_l2(self, step, seed):
        ...
        self._l2_transition_step = step
        dump_state(self, 'L2->L3', step, seed)

    def on_l3(self, step, seed):
        ...
        self.l4_targets = []
        dump_state(self, 'L3->L4', step, seed)
...
def main():
    ...
    t_start = time.time()
    print("Step 611: LS20 L2->L3 state diagnostic", flush=True)
    print(f"  Dumping state at L1->L2 and L2->L3 transitions", flush=True)
        ...
        for step in range(1, 500_001):
            ...
            if time.time() - t_seed > per_seed_cap:
                print(f"  cap @{step} go={go} l2c={sub.l2_count} l3c={sub.l3_count} "
                      f"puq_wall={'built' if sub._puq_wall_set else 'None'} "
                      f"wp={sub._puq_wp_idx} deaths={sub._puq_death_count}", flush=True)
                break

        R.append({'seed': seed, 'l2c': sub.l2_count, 'l3c': sub.l3_count,
                  'l3_steps': l3_steps, 'puq_wall': sub._puq_wall_frozen,
                  'l3_frames_at_first_l2': None})

    print(f"\n{'='*60}", flush=True)
    print(f"Step 611 SUMMARY:", flush=True)
    l3_wins = sum(1 for r in R if r['l3c'] > 0)
    print(f"  L3 reached: {l3_wins}/{len(R)}", flush=True)
    for r in R:
        print(f"  s{r['seed']}: L2={r['l2c']} L3={r['l3c']} "
              f"wall={r['puq_wall']} L3@{r['l3_steps'][:3]}", flush=True)
    print(f"  total_t={time.time()-t_start:.1f}s", flush=True)

    print(f"\nKEY FINDING:", flush=True)
    print(f"  Check dumps above: at L2->L3, is puq_wall_set None?", flush=True)
    print(f"  If l3_frames=0 at first L2->L3, wall map can't be built.", flush=True)
    print(f"  Fix: build wall_set from L2 mode (not L3) for initial PUQ nav,", flush=True)
    print(f"       or use argmin until l3_frames >= WARMUP.", flush=True)
