"""
step0943_pixel_variance.py -- Running pixel variance as observation preprocessing.

R3 hypothesis: EMA of (obs - ema_obs)^2 builds accumulated volatility map.
Which regions CONSISTENTLY change get high variance; static walls stay low.
Alpha on variance-features = attention to reliably-changing regions (R3: modifying
HOW the substrate sees, not what it stores).

Difference from 942 (frame diff):
  942: instantaneous change (this step vs last)
  943: accumulated volatility (which regions change OFTEN)
  Variance builds over ~100 steps. Agent corridor in LS20 gets high variance,
  static background stays low.

ONE VARIABLE CHANGE from 916: feed ema_var through _enc_frame instead of obs.
Everything else identical (alpha, W_pred, h, 800b delta selector).

ema_obs = 0.99 * ema_obs + 0.01 * obs
ema_var = 0.99 * ema_var + 0.01 * (obs - ema_obs)^2
Feed ema_var to _enc_frame.
Init: ema_obs = first obs, ema_var = zeros. Reset at each game start.
...
ALPHA_LO = 0.10
ALPHA_HI = 5.00
VAR_DECAY = 0.99       # EMA decay for running variance
...
class Chain943:
    """916 architecture. Preprocessing: feed running pixel variance to _enc_frame.

    ema_obs + ema_var: reset per game. Everything else 916-identical.
    """
    ...
    def __init__(self, seed=0):
        ...
        self.ema_obs = None    # running mean of obs (pixel-wise), reset per game
        self.ema_var = None    # running variance of obs (pixel-wise), reset per game
        self._prev_ext = None
    ...
    def set_game(self, n_actions):
        ...
        self.ema_obs = None   # init on first observation
        self.ema_var = None
        self._prev_ext = None
        self._prev_action = None
        self._phase_step = 0
    ...
    def reset_seed(self, seed):
        ...
        self.h = np.zeros(H_DIM, dtype=np.float32)
        self.delta_per_action = None
        self.ema_obs = None; self.ema_var = None
        self._prev_ext = None; self._prev_action = None
        self._phase_step = 0; self._diag_log = {}

    def _encode(self, obs):
        obs_f = np.asarray(obs, dtype=np.float32)
        # Running pixel variance
        if self.ema_obs is None:
            self.ema_obs = obs_f.copy()
            self.ema_var = np.zeros_like(obs_f)
        else:
            self.ema_obs = VAR_DECAY * self.ema_obs + (1 - VAR_DECAY) * obs_f
            self.ema_var = VAR_DECAY * self.ema_var + (1 - VAR_DECAY) * (obs_f - self.ema_obs) ** 2

        enc_raw = _enc_frame(self.ema_var)
        self._n_obs += 1
        a = 1.0 / self._n_obs
    ...
    def process(self, obs):
        ...
        if self._phase_step in DIAG_STEPS:
            var_mean = float(self.ema_var.mean()) if self.ema_var is not None else 0.0
    ...
    def on_level_transition(self):
        self._prev_ext = None
        self._prev_action = None
        # Keep ema_obs/ema_var across level transitions (accumulated volatility)
...
print("=" * 70)
print("STEP 943 — RUNNING PIXEL VARIANCE PREPROCESSING")
print("=" * 70)
print(f"One variable change from 916: feed ema_var (decay={VAR_DECAY}) to _enc_frame.")
...
print(f"\n{'=' * 70}")
print(f"STEP 943 RESULTS (pixel-variance, PRISM-light, 10K/phase, {len(TEST_SEEDS)} seeds):")
