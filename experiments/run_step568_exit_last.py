"""
Step 568 — Visit all targets before exit. Greedy TSP, exit forced last.

Builds on Step 567 (L1=5/5 avg 468 steps, 8 rare targets).
Key: identify exit target from first L1 success, then always visit it last.
Visits all 7 non-exit targets first -> one is the energy palette -> L2.

Kill: L2=0/5 -> palette not in rare targets OR 8-target visit >129 steps.
Find: L2>0 -> FIRST L2 EVER (experiment 568).
...
def greedy_order(targets, ay, ax, exit_idx=None):
    """Greedy nearest-first order; if exit_idx set, force that target last."""
...
class RecodeExitLast:
    def __init__(self, k=K, seed=0):
        ...
        self.n_frames = 0
        self.targets = []
        self.exit_idx = None        # which target is the exit
        self.visit_order = []       # planned order for this episode
        self.visit_ptr = 0          # current position in visit_order
        self.agent_yx = None
        self.prev_arr = None
        ...
        self._last_visited_idx = None  # track which target was last visited
    ...
    def act(self):
        if self._steps_since_detect >= REDETECT_EVERY and self.n_frames >= WARMUP:
            ...
            # Rebuild visit order for this episode
    ...
    def on_reset(self):
        ...
        self._steps_since_detect = REDETECT_EVERY
        # Reset visit plan for new episode
        if self.targets and self.agent_yx is not None:
            self.visit_order = greedy_order(self.targets, 32.0, 32.0, self.exit_idx)
        self.visit_ptr = 0
...
def t0():
    # find_rare_clusters test
    mode = np.zeros((64, 64), dtype=np.int32)
    mode[10:12, 10:15] = 7  # 10px rare cluster
    clusters = find_rare_clusters(mode)
    assert any(c['color'] == 7 for c in clusters)
    assert all(c['color'] != 0 for c in clusters)
    # greedy_order: exit_idx=0, should be last
...
def main():
    ...
    for seed in range(n_seeds):
        ...
        env = mk(); sub = RecodeExitLast(seed=seed*1000)
            ...
            if cl > level:
                ...
                if cl == 1 and l1 is None:
                    ...
                    print(f"  s{seed} L1@{step} exit_idx={sub.exit_idx} "
                          f"tgt={sub.n_targets_found} go={go}", flush=True)
                if cl == 2 and l2 is None:
                    l2 = step
                    print(f"  s{seed} L2@{step}!! FIRST L2! exit_idx={sub.exit_idx} "
                          f"tgt={sub.n_targets_found}", flush=True)
            if step % 25_000 == 0:
    ...
    if l1s: print(f"Avg L1: {np.mean(l1s):.0f} steps (567 baseline: 468)")
    if l2n > 0:
        print(f"\n*** FIND: L2={l2n}/{len(R)}! FIRST L2 EVER! ***")
        print("Mode map + exit-last TSP successfully navigates to energy palette!")
    elif l1n >= 2:
        print(f"\nL1={l1n}/{len(R)}, L2=0. Palette not among rare targets OR budget too tight.")
        print("Try: increase visit distance, check if palette triggers energy refill when touched.")
    else:
        print(f"\nKILL: L1={l1n}/{len(R)}. Exit-last strategy breaks navigation.")
